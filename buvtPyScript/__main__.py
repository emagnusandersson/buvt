...
def parsePairList(path, strStart='MatchingData'):
  ...
  for i, strRow in enumerate(arrInp):
    ...
    if(mode==EnumMode.LookForStart):
      if(strRow==strStart): mode=EnumMode.LookForA; strMeta=strRow; continue
...
def parseRelations(strData, boAssignObj=False, funCreateKey=lambda arr: '_'.join(arr), funSide=lambda arr: arr[0]=='T'):
  ...
  for strRow in arrInp:
    ...
    if(strFirst=="MatchingData"):
      ...
      arrPartM=strPost.split(None, nSplitM); nPartM=len(arrPartM)
...
def renameFiles(**arg):
  ...
  for i, row in enumerate(arrRename):    # Rename to final names
    fsTmp=arrTmpName[i]; fsOld=row["strA"]
    fsTmp2=fsTmp if boViaTmpName else fsOld
    fsNew=row['strB']
    try: shutil.move(fsTmp2, fsNew)
    except FileNotFoundError as e:
      return {"e":e, "strTrace":myErrorStack(e.strerror)}   
  return None
...
def getId(**arg): # fsFile contains SSV with 'strType' and 'strName'
  ...
  for row in arrEntry:
    ...
    #print(f'{boFile} {boDir} {st.st_ino} {st.st_mtime_ns:019} {st.st_size}')
    print(f'{st.st_ino}')
...
def copyLocally(**arg): # fsFile contains MatchingData
  ...
  err, objS, objT, _, _=parseRelations(strData, True)
  #n=len(arrRename); arrTmpName=[""]*n
  if(err): return err

  for key, arrS in objS.items():
    arrT=objT[key]
    if(len(arrS)!=1): return {"e":e, "strTrace":myErrorStack('len(arrS)!=1')}
    fsS=arrS[0]["strName"]
    for rowT in arrT:
      fsT=rowT["strName"]; mtime=int(rowT["mtime"])
      try:
        shutil.copy2(fsS, fsT, follow_symlinks=False) # copy2 will perserve meta-data (unlike for example copy)
      except Exception as e:
        return {"e":e, "strTrace":myErrorStack(e.strerror)}
      if("mtime" in rowT):
        os.utime(fsT, ns=(mtime, mtime), follow_symlinks=False)
