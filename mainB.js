...
var funLoad=async function(){
  ...
  var [err, boReZip] = await libPythonSyncZip(); if(err) {debugger; return [err]}
  ...
  gThis.butClear=createElement('button').myAppend('C').prop({title:'Clear output console (and match results)'}).css({position: 'absolute', right: '0px', bottom:'18px', 'z-index':1}).on('click',  function(){
    myConsole.clear();
  ...
  elConsoleContainer.myAppend(elConsoleContainerTop, dragHR, butClear, elTerminal); //, divConsole
