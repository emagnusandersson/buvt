...
gThis.divTopCreator=function(el){
  ...
  el.mySetLinkNLabel=async function(){
    ...
    var p=myRealPath(fiDirSource+'/'+leafDb); //if(err) {debugger; myConsole.error(err); return;}
    var arr=await p;
    ...
    var [err, fsFilterFirst]=await myRealPath(fiDirSource+'/'+leafFilterFirst); if(err) {debugger; myConsole.error(err); return;}
    ...
    var [err, fsFilterFirstTree]=await myRealPath(fiDirSource+'/'+leafFilterFirstTree); if(err) {debugger; myConsole.error(err); return;}
  ...
  var linkFolder=createElement('a').myAppend('App data folder').prop({href:'', title:fsDataHome}).on('click',  async function(ev){
    ev.preventDefault();
  ...
  el.myAppend(...ButTab, ' ', linkFolder, ' | ', linkWiki, ' | ', linkNotation, ' | ', selectorOfTheme);  //, imgBusy , butClear .addClass('footDiv') , settingButton, , divNotations, ' | ', linkDb
...
var makeOpenExtCB=function(key){
  var cbFunI=async function(ev){
    ev.preventDefault()
    var arrCommand=[strExec, FsFile[key]]
    var strCommand=arrCommand.join(' ');
...
gThis.divSMTabCreator=function(el, ...Fun){
  ...
  var htmlBody=
  `<tr><th>0</th><td>(∞)</td><td><span>-</span></td> <td><span>-</span><button>-</button></td> <th><span>-</span></th></tr>
  <tr><th>1</th><td><span>-</span></td><td><span>-</span><button>-</button></td>  <td><span>-</span><button>-</button></td></tr>
  <tr><th>Many</th><td><span>-</span><button>-</button></td>  <td><span>-</span><button>-</button></td>  <td><span>-</span><button>-</button></td><th><span>-</span> \\</th></tr>
  <tr><td rowspan=2>∑</td><th><span>-</span></th><td></td><th>\\ <span>-</span></th></tr>
  <tr><th colspan=3><span>-</span>(<span>-</span>) \\ <span>-</span>(<span>-</span>) (<span>-</span>/<span>-</span>)</th></tr>`
  ...
  var But0=tr0.querySelectorAll('button'), [but02]=But0
  var arr1=tr1.querySelectorAll('span'), [span10,span11,span12]=arr1
  var But1=tr1.querySelectorAll('button'), [but11,but12]=But1
  var arr2=tr2.querySelectorAll('span'), [span20,span21,span22, span2XSum]=arr2
  var But2=tr2.querySelectorAll('button'), [but20,but21,but22]=But2
  ...
  var But=[...But0, ...But1, ...But2];  But.forEach(ele=>ele.addClass('smallBut'))
...
gThis.divMatWrapCreator=function(el, ...Fun){
  el.clearVal=function(){
    divTab.clearVal()
    hovDetail.myText(`- \\ -, M: -(-) \\ -(-) (-)`)
  }
  ...
  divSMTabCreator(divTab, ...Fun); //, undefined, fun
  divTab.css({background:'var(--bg-color)', width:'fit-content', padding:'1px', border:"4px solid var(--border-color)"}); // 'font-size':'0.9rem', 
  ...
  var hovDetail=hovHelp.cloneNode(1).css({'margin-left':'0.6em', 'padding':'0 0.3em'}).myText('-');
  //popupHover(hovDetail, divTab);
  var elWrap=createElement('span').myAppend(hovDetail, divTab)
  var elPar=elBody
  myMenu(elWrap, elPar)
...
gThis.divCategoryTabT2DCreator=function(el){
  ...
  el.clearVal=function(){
    ...
    divMat1.clearVal(); divMat2.clearVal()
...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  var [trMat1, trUntouched, trRenamed, trCopiedNRenamedToOrigin, trChanged, trReusedName, trReusedId, trMat2, trCreatedNDeleted, tr1T1, trMult, trSum, trSyncAccordingToFiles]=arrTR
  ...
  //divMatWrapCreator(divMat1, makeOpenExtCB("T2D_STMatch1"));
  divMatWrapCreator(divMat1, makeOpenExtCB("T2D_STMatch1_02"), null, makeOpenExtCB("T2D_STMatch1_12"), makeOpenExtCB("T2D_STMatch1_20"), makeOpenExtCB("T2D_STMatch1_21"), makeOpenExtCB("T2D_STMatch1_22"));
  ...
  butDoActionRam.on('click', async function(){
    ...
    var [err]=await self.syncTreeToDb.createSyncData(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    //var [err]=await T2DCreateSyncData.call(self.syncTreeToDb); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    
    if(boAskBeforeWrite){
      var boOK=confirm(`Overwrite ${self.syncTreeToDb.fsDb}`);  if(!boOK) {busyLarge.hide(); return}
    }

    //var [err]=await T2DMakeChanges.call(self.syncTreeToDb)
    var [err]=await self.syncTreeToDb.makeChanges()
    myConsole.log('Do actionsRam: Done'); busyLarge.hide();
  })
  butDoAction.on('click', async function(){
    busyLarge.show();
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    var args=copySome({}, result, ["charTRes", "leafFilterFirst", "flPrepend", "leafDb"])
    var [err, fsDirSource]=await myRealPath(result.fiDirSource); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    ...
    var [err]=await syncTreeToDb.readAction(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    var [boAbort, strMess]=syncTreeToDb.getMess()
    if(boAbort) {myConsole.printNL('Nothing to do, aborting'); busyLarge.hide(); return;}
    var [err]=await syncTreeToDb.doSCChanges(); if(err) {myConsole.error(err); busyLarge.hide(); return;}
    var [err]=await syncTreeToDb.createSyncData(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    if(boAskBeforeWrite){
      var boOK=confirm(`Overwrite ${syncTreeToDb.fsDb}`);   if(!boOK) {busyLarge.hide(); return}
    }
    var [err]=await syncTreeToDb.makeChanges(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}

    myConsole.log('Do actions: Done'); busyLarge.hide(); 
  });
  butDoActionRam.hide()
...
gThis.divT2DCreator=function(el){
  ...
  var butCompareTreeToDb=createElement('button').myAppend('Compare').on('click', async function(){
    ...
    myConsole.log('Compare tree to Db: Done'); busyLarge.hide()
  });
  var butSyncTreeToDb=createElement('button').myAppend('Sync').prop({title:`Same as "Compare" plus doing the suggested actions from the compare-call.`}).on('click', async function(){
    ...
    var [err]=await syncTreeToDb.createSyncData(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    //var [err]=await T2DCreateSyncData.call(syncTreeToDb); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    if(boAskBeforeWrite){
    ...
    myConsole.log('Sync tree to Db: Done'); busyLarge.hide();
  });
  ...
  el.clearVal=function(){
    ...
    spanNTree.myText(`- \\ -`).prop({title:strTitleDefault})
    butHL.prop({title:strTitleDefault});
    butHL.myText('-(-)')
    //butMore.myText('-')
  }
  el.setVal=function(syncTreeToDb){
    ...
    // var strTitle=`Files: nName(nId): ${nTreef}(${nIdf})\nFolders: nName(nId): ${nTreeF}(${nIdF})`
    // spanNTree.myText(`${nTreef}`).prop({title:strTitle})
    // var strTitle=`Db: nName(nId): ${nDb}(${nIdDb})`
    // spanNDb.myText(`${nDb}`).prop({title:strTitle})

    var strTitle=`______nName(nId)\nFiles: ${nTreef}(${nIdf})\nFolders: ${nTreeF}(${nIdF})\nDb: ${nDb}(${nIdDb})`
    spanNTree.myText(`${nTreef} \\ ${nDb}`).prop({title:strTitle})


    var strTitle=`______nName(nId)\nFiles: ${nMultf}(${nMultIdf})\nFolders: ${nMultF}(${nMultIdF})\nDb: ${nMultDb}(${nMultIdDb})`
  ...
  el.linkFilterFirst=createElement('a').myText('filter').prop({ href:""}).on('click',  async function(ev){
    ev.preventDefault();
  ...
  var htmlHL=`<b title="Hard Link-check\n(Hard links not allowed.)">HL-check:</b> <span>-</span>. Id-match: <span>-</span>. Multiples: <a href="" >-</a>` 
  ...
  var [spanHLCheckResult, spanNTree]=Span; //, spanNDb
...
<tr><td colspan=8>  <div class=dupEntry>SM-Match: </div>  </td></tr>
<tr><th>-</th><th>1T1</th> <td colspan=2><a href="">-</a> <a href="">Compare</a></td><td>1T1</td>  <td><span>-</span> <button>Rename</button></td> <td colspan=2 class=crossed></td></tr>
...
<tr><th>-</th><th>ViaA</th> <td colspan=2><a href="">-</a></td><td><font title="Identified via 1T1-files only renamed in ancestor">ViaA</font></td>  <td><span>-</span> <button>Rename</button></td> <td colspan=2 class=crossed></td></tr>
<tr><td colspan=8>  <div class=dupEntry>SM-Match: </div>  </td></tr>
...
//<button>Delete</button> <button>Create</button>   <br/>(read fr files)
  ...
  var [,,,,thAction]=tHeadRA.children; thAction.css({'text-align':'center'})
  var arrTR=[...tBody.children]
  var [trMat1, trUntouched, trMat2, tr1T1, trViaAExtra, trViaA, trMat3, trChanged, trCreatedNDeleted, trMult, trSum, trDeleteNCreate]=arrTR
  ...
  //divMatWrapCreator(divMat1, makeOpenExtCB("T2T_STMatch1"));
  divMatWrapCreator(divMat1, makeOpenExtCB("T2T_STMatch1_02"), null, makeOpenExtCB("T2T_STMatch1_12"), makeOpenExtCB("T2T_STMatch1_20"), makeOpenExtCB("T2T_STMatch1_21"), makeOpenExtCB("T2T_STMatch1_22"));
  //divMatWrapCreator(divMat2, makeOpenExtCB("T2T_STMatch2"));
  divMatWrapCreator(divMat2, makeOpenExtCB("T2T_STMatch2_02"), null, makeOpenExtCB("T2T_STMatch2_12"), makeOpenExtCB("T2T_STMatch2_20"), makeOpenExtCB("T2T_STMatch2_21"), makeOpenExtCB("T2T_STMatch2_22"));
  divMatWrapCreator(divMat3); // , makeOpenExtCB("T2T_STMatch3")
  ...
  but1T1Compare.prop('title',`Compare ${LeafFile.T2D_renamed} and  ${LeafFile.T2T_renamed} (Opens in Meld.)`).css({'font-size':"0.85em"}).on('click',  async function(ev){
    ev.preventDefault()
    var strExec='meld'
  ...
  butDeleteNCreateRam.on('click', async function(){
    busyLarge.show();
    var [err]=await self.syncTreeToTree.makeDeleteNCreateChanges();  if(err) {debugger; busyLarge.hide(); return;}
    myConsole.log('DeleteNCreateRam: Done'); busyLarge.hide();
  })
  butDeleteNCreate.on('click', async function(){
    //var [err]=await self.syncTreeToTree.makeDeleteNCreateChanges();  if(err) {debugger; busyLarge.hide(); return;}
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    var {fiDirSource, fiDirTarget}=result
    var doDCActionsToTree=new DoDCActionsToTree({fiDirSource, fiDirTarget});
    var [err, {nDelete, nCreate}]=await doDCActionsToTree.read(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    if(nDelete==0 && nCreate==0) {myConsole.printNL('Nothing to do, aborting'); busyLarge.hide(); return;}
    var strMess=doDCActionsToTree.getMess();
    var boOK=confirm(strMess)
    if(!boOK) { busyLarge.hide(); return; }
    busyLarge.show();
    var [err]=await doDCActionsToTree.makeChanges(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    myConsole.log('DeleteNCreate: Done'); busyLarge.hide();
  });
...
gThis.divT2TCreator=function(el){
  ...
  var funTreeToTree=async function(){
    ...
    var [err]=await syncTreeToTree.constructorPart2();   if(err) { myConsole.error(err); busyLarge.hide(); return; }
    var [err]=await syncTreeToTree.compare();   if(err) { myConsole.error(err); busyLarge.hide(); return; }
    ...
    if(boSync){
      if(!syncTreeToTree.boChanged){ myConsole.log('Everything is up to date, aborting.'); busyLarge.hide(); return;  }
      if(boAskBeforeWrite){
        var strMess=syncTreeToTree.makeConfirmMess()
        var boOK=confirm(strMess);
        if(!boOK) {busyLarge.hide(); return}
      }
      var [err]=await syncTreeToTree.makeChanges();   if(err) { debugger; myConsole.error(err); busyLarge.hide(); return; }
  ...
  var butSyncTreeToTreeBrutal=createElement('button').myAppend('Sync (like rsync)').css({position:'absolute', right:'0px'}).attr({'title':'Files with the same name, size \nand mod-time are left untouched.\nOthers are overwritten\n(Renamed files are overwritten.)'}).on('click', async function(){
    busyLarge.show(); //myConsole.clear(); el.clearVal(); 
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); busyLarge.hide(); return;}
    ...
    var [err, boChanged]=await syncTreeToTreeBrutal.compare(); if(err) { myConsole.error(err); busyLarge.hide(); return;}
    ...
    myConsole.log('Sync tree to tree (like rsync) Done'); busyLarge.hide();
  });


  var hT2T=createElement('b').myText('Tree to tree');
  var divButton=createElement('div').myAppend(hT2T, ' ', butCompareTreeToTree, butSyncTreeToTree,' ', el.linkFilterFirstTree, butSyncTreeToTreeBrutal).css({background:'var(--bg-color)', flex:"0 1", top:0, border:"solid 1px"}); //, butWriteMTimeOnSource , position:'sticky'
...
REMEMBER to run main>sync berfore "checking". Otherwise changed/missing files will be reported an errors.`})
  ...
  var divMid=createElement('div').myAppend(divHeadCheckHash, butCheckS, butCheckT).css({display:'flex', 'column-gap':'3px', margin:'3px 0', background:"var(--bg-color)"}); //, butHardLinkCheck, butSetLastCheck  hCheckFileExistance, butCheckSummarizeMissingS, butCheckSummarizeMissingT, 
...
gThis.ArgumentTabTdTLastSync={  
  ...
  setTLastSync:async function(){ // this will refer to the button
    ...
    if(!boOK) { return}
    var  [err]=await elR.setTLastSync(); if(err) {myConsole.error(err); return [err]};
    this.myText('0 s')
  },
  factory:function(){ 
...
gThis.ArgumentTabTdTLastCheck={
  ...
  setTLastCheck:async function(){ // this will refer to the button
    ...
    var boOK=confirm(`Setting tLastCheck?\nIt can be undone (set directly) in the file:\n ${fsMyStorage}`);
    if(!boOK) { return}
    var  [err]=await elR.setTLastCheck(); if(err) {myConsole.error(err); return [err]};
    this.myText('0 s')
  },
  factory:function(){ 
...
gThis.ArgumentTabRow={
  ...
  setTLastSync:async function(){
    ...
    var [err]=await argumentTab.saveToDisk(); if(err) return [err];
    return [null]
  },
  ...
  rDefault:{ind:0, boSelected:false, label:'', fiDirSource:'', fiDirTarget:'', leafFilterFirst:'', leafFilterFirstTree:'', leafDb:'', flPrepend:'', charTRes:'9', tLastSync:0, tLastCheck:0},
  factory:function(){
...
gThis.argumentTabExtend=function(el){
  ...
  el.setUpDom=async function(){
    var [err, ObjOpt, iSelected]=await el.getDataFrFile();  if(err) return [err];
  ...
  //   var [err, rowSelected, iSelected]=await el.getSelectedFrDom();  if(err) return [err];
  //   rowSelected.dataset.tLastSync=unixNow();
  //   var [err]=await el.saveToDisk(); if(err) return [err];
  //   return [null]
  // }
  ...
  var myStorage=new MyStorage(fsMyStorage)
...
// openInBrowserF("https://neutralino.js.org/docs");
