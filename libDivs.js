...
gThis.divCategoryTabT2DCreator=function(el){
  ...
  el.setVal=function(syncTreeToDb){
    ...
    //var {Mat1, Mat2, nUntouched, nIM, nNM, nChanged, nReusedName, nReusedId, nCreate, nDelete, n1T1NoName, nSourceMultNoName, nTargetMultNoName, nSource, nTarget, boChanged}=syncTreeToDb


    var {Mat1, Mat2, boChanged, arrSource, arrTarget, arrSourceUntouched, arrTargetUntouched, arrSourceChanged, arrTargetChanged, arrSourceIM, arrTargetIM, arrSourceNM, arrTargetNM, arrSourceReusedName, arrTargetReusedName, arrSourceReusedId, arrTargetReusedId, arrCreate, arrDelete, arrSource1T1NoName, arrTarget1T1NoName, arrSourceMultNoName, arrTargetMultNoName, ArrSourceMultNoName, ArrTargetMultNoName}=syncTreeToDb
    ...
    butRenamed.myText(nIM);  spanRenamedAction.myText(nIM)
    butCopiedNRenamedToOrigin.myText(nNM);  spanCopiedNRenamedToOriginAction.myText(nNM)
...
gThis.viewFrontCreator=function(el){
  ...
  el.setUp=function(){
    ...
    elConsoleContainerTop.innerHTML=""
  }
  ...
  var divMid=createElement('div').myAppend(divT2D, divT2T).css({display:'grid', 'grid-template-columns':'1fr 1fr', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'1 1 auto', 'overflow-y':'auto'}); //.addClass('unselectable')
...
gThis.viewCheckCreator=function(el){
  ...
  var butCheckViaPythonT=createElement('button').myAppend('Target').on('click', async function(){
    ...
    //viewCheck.butCheckCancel.show(); viewCheck.boCheckCancel=false
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); return;}
    var {fiDirTarget, charTResT2T, strHostTarget:strHost}=result
    var arg=extend({}, {fiDir:fiDirTarget, iStart:Number(inpIStart.value), myConsole, charTRes:charTResT2T, strHost})
    var [err, mess=""]=await checkViaPython(arg); if(err) {debugger; myConsole.error(err); }
    setMess(mess); viewCheck.butCheckCancel.hide();
  });
...
gThis.argumentTabExtend=function(el){
  ...
  el.setUp=async function(){
    divTopContainer.myAppend(divTop)
    elConsoleContainerTop.myAppend(footDiv)
  ...
  var footDiv=createElement('div').myAppend(buttonAdd, buttonSaveSortOrder).addClass('footDiv').css({width:'100%',background:"var(--bg-colorEmp)", margin:'0px', padding:'0.4em', 'border-top':'solid 1px'}); //, spanSpace buttonBack, , spanLabel
  //var divConsoleContainer=createElement('div').css({width:'100%', opacity:0.8, 'text-align':'left'});
  //var footDivW=createElement('div').myAppend(footDiv).addClass('footDivW').css({position:'fixed', bottom:'0px', opacity:0.8, 'flex-direction':'column', background:'var(--bg-color)'}); //divConsoleContainer
...
class MyConsoleTerm{
  ...
  error(str){
    ...
    var strRed=`\x1B[1;3;42;31m${str}\x1B[0m`
