...
gThis.LocationRow={
  ...
  mySetDomRow:function(rowData){
    ...
    var elR=this;
      // Copying, since rowData points back into cache.
    //extend(elR.dataset, rowData); // dataset stringifies things, so it won't matter that rowData points back into cache.
    var rowDataCopy=copyShallow(rowData); // Assumes rowData only contains primitive types
    this.parentNode.wmRowData.set(elR, rowDataCopy)
    //elR.prop('rowData',rowData)
    for(var i=0;i<KeyPropWInd.length;i++) { 
      var name=KeyPropWInd[i], val=rowDataCopy[name]; //, td=this.children.item(i)
  ...
  //   this.parentNode.wmRowData.get(this).tLastSync=unixNow();
  ...
  setTLastCheck:async function(){
    ...
    this.parentNode.wmRowData.get(this).tLastCheck=unixNow();
    var [err]=await locationTab.saveToDisk(); if(err) {debugger; return [err];}
    return [null]
  },
  getTypedData:function(){
    ...
    var objO=copyShallow(this.parentNode.wmRowData.get(this))
    for(var i=0;i<KeyPropWIdInd.length;i++){
      ...
      else if(StrPropType[strName]=='boolean') val=castToBoolean(val)
      objO[strName]=val
    }
    return objO
  },
  propDefaultWIdInd:{ind:0, id:0, label:'', fiDir:'', strHost:'', charTRes:'9', tLastCheck:0, charFilterMethod:"b", boAllowsLinks:true, boAllowsCaseCollision:true, strCharSet:'ext4'}, //, tLastSync:0
  factory:function(){
...
var StrHeaderTitle={ind:'Current sortorder', label:'', 
  ...
  strHost:'Host (empty means localhost)\nNote! if this field is non-empty, then the Location can only be used as target.',
...
Note 1: You can compare the filtering methods in More>"Compare filtering methods".
Note 2: in T2T-top-level the "suffixFilterFirstT2T"-setting is added to the filter-file-name (so .buvt-filterWHATEVER or .rsync-filterWHATEVER)`,
  boAllowsLinks:`If file system can handle (soft) links.
Ex:
ext4 and ntfs CAN handle soft links
fat32 and exfat CAN NOT handle soft links`,
  boAllowsCaseCollision:`If the file system can handle multiple files that only differs in case, such as "A.txt" and "a.txt"
Ex:
ext4 and ntfs CAN handle case collisions
fat32 and exfat CAN NOT handle case collisions`,
  strCharSet:`What character-set the filesystem can handle:
  ext4: reserved characters: "/"
  ms: reserved characters: ""*/:<>?\\|" (Microsoft (NTFS, exFAT, FAT32 (VFAT)))
When syncing from source to target and the file-name contains characters that are unsupported on the target side, then that file si skipped. Although they are listed under "Main">"T2T-section">"Skipped".`
}
...
gThis.locationSetPopExtend=function(el){
  ...
  var save=async function(){ 
    ...
    if(label.length==0) {myConsole.error(Error(`label is empty`)); return};
    if(locationTab.isPropInTab('label', label)) {myConsole.error(Error(`label already exists`)); return};
  ...
  el.setUp=function(){
    ...
    inpAllowsLinks.checked=castToBoolean(rowDataLoc.boAllowsLinks)
    //inpAllowsCaseCollision.checked=rowDataLoc.boAllowsCaseCollision; //=="true"
    inpAllowsCaseCollision.checked=castToBoolean(rowDataLoc.boAllowsCaseCollision)
  ...
  el.openFunc=function(boUpdT){
    ...
    if(this==null){rowDataLoc=extend({},LocationRow.propDefaultWIdInd);}
      ...
      //rowDataLoc=extend({},elR.dataset); // Note! In dataset all properties are of type string
      rowDataLoc=elR.parentNode.wmRowData.get(elR)
      if(!boUpd) rowDataLoc=copyShallow(rowDataLoc)
    }
...
gThis.locationDeletePopExtend=function(el){
  ...
  el.openFunc=function(){
    elR=this.parentNode.parentNode;
    //var rowDataLoc=elR.dataset
    var rowDataLoc=elR.parentNode.wmRowData.get(elR)
    spanLabel.myText(rowDataLoc.label);
...
gThis.locationTabExtend=function(el){
  ...
  el.setIndToCurrentOrder=function(){
    for(var i=0;i<tBody.childElementCount;i++) {
      var elR=tBody.children[i];
      //var rowDataLoc=elR.dataset
      var rowDataLoc=wmRowData.get(elR);
      rowDataLoc.ind=i
    } 
  }
  ...
  el.getRowFrDomById=function(id){
    if(typeof id!='Number') return [Error(`typeof id!='Number'`)];
    var row=undefined, iId=NaN;
    for(var i=0;i<tBody.childElementCount;i++){
      var rowA=tBody.children[i];
      //var rowDataLoc=rowA.dataset
      var rowDataLoc=wmRowData.get(rowA);
      var boMatch=rowDataLoc.id==id
  ...
  // el.setTLastSync=async function(id){ debugger // This function is never used
  //   var [err, row, iId]=el.getRowFrDomById(id);  if(err) {debugger; return [err];}
  //   //var rowDataLoc=row.dataset
  //   var rowDataLoc=wmRowData.get(row);
  //   rowDataLoc.tLastSync=unixNow();
  ...
  el.getHighestProp=function(key){
    ...
    Row.forEach(function(rowA){
      //var rowDataLoc=rowA.dataset
      //var v=Number(rowDataLoc[key])
      var rowDataLoc=wmRowData.get(rowA);
      var v=rowDataLoc[key]
      if(v>vHighest) vHighest=v;
    })
    return vHighest
  }
  el.isPropInTab=function(k, vIn){ 
    var boFound=false, Row=[...tBody.children];
    for(var i=0;i<Row.length;i++){
      var rowA=Row[i], rowDataLoc=wmRowData.get(rowA), v=rowDataLoc[k]
      if(v==vIn) {boFound=true; break;}
    }
    return boFound
  }
  ...
  var wmRowData=tBody.wmRowData=new WeakMap()
