...
var StrHeaderTitle={ind:'Current sortorder', key:'', 
  ...
  strHost:'Host of the location (empty means localhost)',
...
Note 2: in T2T-top-level the "suffixFilterFirstT2T"-setting is added to the filter-file-name (so .buvt-filterWHATEVER or .rsync-filterWHATEVER)`,
  boAllowsLinks:`boAllowsLinks: If the target can handle (soft) links.`,
  boAllowsCaseCollision:`boAllowsCaseCollision: If target can handle multiple files that only differs in case, such as "A.txt" and "a.txt"`,
...
gThis.locationSetPopExtend=function(el){
  ...
  var labKey=createElement('div').myAppend('Key').prop({title:'Key (label) Displayed on the tab.'});
  var labDir=createElement('div').myAppend('fiDir').prop({title:StrHeaderTitle.fiDir});
  var labHost=createElement('div').myAppend('Target host').prop({title:StrHeaderTitle.strHost});
  var labTRes=createElement('div').myAppend('TRes').prop({title:StrHeaderTitle.charTRes});
  ...
  var labAllowsLinks=createElement('div').myAppend('Allow links').prop({title:StrHeaderTitle.boAllowsLinks});
  var labAllowsCaseCollision=createElement('div').myAppend('Allow case-collisions').prop({title:StrHeaderTitle.boAllowsCaseCollision});
  var labCharSet=createElement('div').myAppend('Target character set').prop({title:StrHeaderTitle.strCharSet});
