...
gThis.ArgumentTabRow={
  ...
  mySetDomRow:function(rowData){
    var {StrColOrder}=this
    var {boSelected, label, fiSourceDir, fiTargetDbDir, fiTargetDataDir, suffixFilterFirstT2T, charTResS, charTResT, charTResCollision, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strTargetCharSet}=rowData; //, leafFilterFirst, leafDb
    var elR=this;
  ...
  rDefault:{ind:0, boSelected:false, label:'', fiSourceDir:'', fiTargetDbDir:'', fiTargetDataDir:'', suffixFilterFirstT2T:'', charTResS:'9', charTResT:'9', charTResCollision:'d', tLastSync:0, tLastCheckS:0, tLastCheckT:0, strHostTarget:'', charFilterMethod:"b", boIncIdS:false, boIncIdT:false, boAllowLinks:true, boAllowCaseCollision:true, strTargetCharSet:'ext4'}, // , leafFilterFirst:'', leafDb:''
  factory:function(){
...
var StrHeaderTitle={ind:'Current sortorder', boSelected:'', label:'', 
  ...
  fiTargetDataDir:`The location of the target tree containing the actual files`,
...
gThis.argumentSetPopExtend=function(el){
  ...
  var save=async function(){ 
    ...
    var fiTargetDataDir=inpTargetDataDir.value; //if(fiTargetDataDir.length==0){ myConsole.log('empty fiTargetDataDir',2);  return;}
    fiSourceDir=rtrim(fiSourceDir, charF)
    fiTargetDbDir=rtrim(fiTargetDbDir, charF)
    fiTargetDataDir=rtrim(fiTargetDataDir, charF)
    ...
    var rT={label, fiSourceDir, fiTargetDbDir, fiTargetDataDir, suffixFilterFirstT2T, charTResS, charTResT, charTResCollision, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strTargetCharSet}; //, leafFilterFirst, leafDb
  ...
  el.setUp=function(){
    ...
    inpTargetDataDir.value=rowDataLoc.fiTargetDataDir
  ...
  var labTargetDataDir=createElement('div').myAppend('fiTargetDataDir').prop({title:StrHeaderTitle.fiTargetDataDir});
  ...
  var Lab=[labLabel, labSourceDir, labTargetDbDir, labTargetDataDir, labFilterFirstT2T, labTResS, labTResT, labTResCollision, labHostTarget, labFilterMethod, labIncIdS, labIncIdT, labAllowLinks, labAllowCaseCollision, labFsTarget];
  var Inp=[inpLabel, inpSourceDir, inpTargetDbDir, inpTargetDataDir, inpFilterFirstT2T, selTResS, selTResT, selTResCollision, inpHostTarget, selFilterMethod, inpIncIdS, inpIncIdT, inpAllowLinks, inpAllowCaseCollision, selFsTarget];
...
gThis.argumentDeletePopExtend=function(el){
  ...
  //label, fiSourceDir, fiTargetDbDir, fiTargetDataDir, leafFilterFirst, suffixFilterFirstT2T, leafDb, charTResS, charTResT, charTResCollision, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strTargetCharSet
gThis.argumentTabExtend=function(el){
  ...
  el.getDataFrFile=async function(){
    ...
    for(var i in ObjOpt){
      ...
      objOpt.fiTargetDataDir=rtrim(objOpt.fiTargetDataDir, charF)
  ...
  el.getSelectedFrFile=async function(){
    ...
    var {strHostTarget, fiTargetDbDir, fiTargetDataDir}=objOptExtended
    ...
    extend(objOptExtended, {strHostTarget, boRemoteTarget, fiTargetDbDir, fiTargetDataDir})
