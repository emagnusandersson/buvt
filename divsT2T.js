...
gThis.divT2TUsingHashCreator=function(el){
  ...
  el.setVal=function(syncT2T){
    ...
    //divAreaCatPrim1.setVal(catPrim1)
  ...
  var hT2T=createElement('b').myText('Tree to tree').prop({title:`Sync source tree (own+upstream) to the target`});
  ...
  var divHead=createElement('div').myAppend(hT2T, ' ', imgTreeS, ` ${charRightArrow} `, imgTreeT, ' ', linkFilterFirstT2T).css({display:'flex', 'align-items':'center', 'column-gap':'3px', background:'var(--bg-color)', flex:"0 1", border:"solid 1px", 'flex-wrap':'wrap', position:'sticky', top:0}); //, hT2T, linkFolder, butWriteMTimeOnSource , butSyncT2T, butSyncT2TBrutal , opacity:0.8

  var butCompareT2T=createElement('button').myAppend('Compare').on('click', funT2T);
  var divCompare=createElement('div').myAppend(butCompareT2T)
  ...
  // var divAreaCatPrim1=createElement('div').myHtml(`SM-Combos: `).hide();
  // divAreaCatPrimCreator(divAreaCatPrim1, makeOpenExtCB(PathT2T.STMatch1_02), null, makeOpenExtCB(PathT2T.STMatch1_12), makeOpenExtCB(PathT2T.STMatch1_20), makeOpenExtCB(PathT2T.STMatch1_21), makeOpenExtCB(PathT2T.STMatch1_22));
  
  var divTab=divTabT2TUsingHashCreator(createElement('div'))
  el.myAppend(divHead, divCompare, divDbCoverageTest, divFolderInfo, divConflict, divTab); // divConsoleT2D, divConsoleT2T, divConsole, divAreaCatPrim1
