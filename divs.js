...
gThis.divTopCreator=function(el){
  ...
  el.mySetLinkNLabel=async function(){
    ...
    // }
    var leafFilter=LeafFilter[charFilterMethod], leafFilterFirstT2T=leafFilter+suffixFilterFirstT2T
    var fiFilterFirstT2T=fiSourceDir+charF+leafFilterFirstT2T
    ...
    linkTDb.prop({title:strTargetDbDir}).toggleClass('disabled', boRemote); //.toggle(!boWNonSeparateTargetDb)
      ...
      // Set S_- Db link
    var fiSourceDb=fiSourceDir+charF+leafDb;   divT2D.linkDb.prop({title:fiSourceDb}).myText(leafDb)
      // Set S_- filter link
    var fiFilterFirstT2D=fiSourceDir+charF+leafFilter;   divT2D.linkFilterFirstT2D.prop({title:fiFilterFirstT2D})

      // Set T_- Db link
    var title=strTargetDbDir+charF+leafDb;   divT2DT.linkDb.prop({title}).myText(leafDb).toggleClass('disabled', boRemote)
      // Set T_- filter link
    var title=strTargetDbDir+charF+leafFilter;   divT2DT.linkFilterFirstT2D.prop({title}).toggleClass('disabled', boRemote)

      // Set T2T-filter links
    //divT2T.linkFilterFirstT2T.prop({title:fiFilterFirstT2T}).toggleClass('disabled', suffixFilterFirstT2T=='');
    divT2TUsingHash.linkFilterFirstT2T.prop({title:fiFilterFirstT2T}).toggleClass('disabled', suffixFilterFirstT2T=='');
  }
...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  butCopiedNRenamedToOrigin.on('click', makeOpenExtCB(PathCur.copiedThenRenamedToOrigin))
...
gThis.divT2DCreator=function(el, charSide){
  ...
  el.linkFilterFirstT2D=createElement('a').myText('filterFirstT2D').prop({ href:""}).on('click', methGoToTitle);
...
gThis.divT2TTabUsingHashCreator=function(el){
  ...
  el.setVal=function(syncT2T){
    ...
    var {ObjFeedback, nExactTot}=syncT2T
...
gThis.ArgumentTabRow={
  ...
  rDefault:{ind:0, boSelected:false, label:'', fiSourceDir:'', fiTargetDir:'', fiTargetDbDir:'', suffixFilterFirstT2T:'', charTResS:'9', charTResT:'9', tLastSync:0, tLastCheck:0, strHostTarget:'', charFilterMethod:"b"}, // , leafFilterFirst:'', leafDb:''
  factory:function(){
