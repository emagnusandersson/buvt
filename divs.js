...
gThis.divTopCreator=function(el){
  ...
  el.setButTab=function(strView){
    ...
    var Views=[viewFront, viewCheck, viewExtra, argumentTab];
  ...
  //butExtra.toggle(gThis.boExperiment)
  ...
  var ButTab=[butFront, butCheck, butExtra, argumentButton]
...
<tr><td colspan=2><a href="">-</a></td><td colspan=2><a href="">-</a></td><td colspan=2><a href="">-</a></td></tr>
...
gThis.miniViewHashMatchCreator=function(el, charSide='S'){
  ...
  var pathHashMultSMUniform=PathCur.hashMultSMUniform, pathHashMultSMNonUniform=PathCur.hashMultSMNonUniform, leafHashMultSMUniform=pathHashMultSMUniform.leaf, leafHashMultSMNonUniform=pathHashMultSMNonUniform.leaf
  var butCheck=createElement('button').myAppend('Check').attr({'title':undefined}).on('click',  async function(){
    myConsole.clear(); setMess(strLab+' ...'); blanket.show();
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, charTResS }=result;
    var arg=extend({}, {fiSourceDir, charTRes:charTResS})
    var [err, result]=await getHashMultSMNonUniform(arg); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform, StrNonUniform, StrUniform}=result

    //var {n, nPatSM, nPatSMMult, nSMMult}=objSM
    aNonUniform.myText(`${nPatNonUniform} (${nTotNonUniform})`).prop({title:StrNonUniform?.join(`\n`)})
    aUniform.myText(`${nPatUniform} (${nTotUniform})`).prop({title:StrUniform?.join(`\n`)})
    setMess(strLab+', Check: Done'); blanket.hide();
  });

  var spanLab=createElement('span').myText('Uniform SM: ')
  var fsSMUniform=pathHashMultSMUniform.fsName, funUniform=makeOpenExtCB(pathHashMultSMUniform);
  var aUniform=createElement('a').prop({href:''}).myAppend(`-`).prop({title:undefined}).on('click',funUniform); //`patterns (files)\n${fsSMUniform}`
  var divHashMultSMUniform=createElement('div').myAppend(spanLab, aUniform)

  var spanLab=createElement('span').myText('Non-uniform SM: ')
  var fsSMNonUniform=pathHashMultSMNonUniform.fsName, funNonUniform=makeOpenExtCB(pathHashMultSMNonUniform);
  var aNonUniform=createElement('a').prop({href:''}).myAppend(`-`).prop({title:undefined}).on('click',funNonUniform); //`patterns (files)\n${fsSMNonUniform}`
  var divHashMultSMNonUniform=createElement('div').myAppend(spanLab, aNonUniform)

  //var strLab='Check for Hash multples with non-uniform SM'
  //var strLab='Files with same hash'
  var strLab='Hash-multiples'
  var head=createElement('h3').css({'text-align':'left'}).myAppend(strLab).prop({title:'... as read from the (source side) db-file'});
  var divResult=createElement('div').css({'text-align':'left'}).myAppend(divHashMultSMUniform, divHashMultSMNonUniform)
  
  el.myAppend(head, butCheck, divResult).css({'text-align':'left'}); // divConsoleT2D, divConsoleT2T, divConsole
...
gThis.miniViewSMMatchCreator=function(el, charSide='S'){
  //var PathCur=gThis[`Path${charSide}`];
  //var pathSMMultHashNonUniform=PathSMMult.smMultHashNonUniform, leafSMMultHashNonUniform=pathSMMultHashNonUniform.leaf
  var butCheck=createElement('button').myAppend('Find').attr({'title':undefined}).on('click', async function(){
    var boTarget=cbSide.checked, charTRes=selTRes.value
    //var [err]=await writeFile(pathSMReserved.fsName, ""); if(err) {debugger; myConsole.error(err); resetMess();  return;}
    myConsole.clear(); setMess(strLab+' ...'); blanket.show();
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    ...
    var arg=extend({}, {fiDir, fiDbDir, charTRes, strHostTarget, boRemote})
    //smMultWork=new SMMultWorkOld()
    smMultWork=new SMMultWork()
    var [err, result]=await smMultWork.getSMMultHashNonUniform(arg); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {StrNonUniform, StrUniform, nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform, nReserved, StrReserved}=result; //, StrMakeUniform, nRewrite

    //var {n, nPatSM, nPatSMMult, nSMMult}=objSM
    aUniform.myText(`${nPatUniform} (${nTotUniform})`).prop({title:StrUniform?.join(`\n`)})
    aNonUniform.myText(`${nPatNonUniform} (${nTotNonUniform})`).prop({title:StrNonUniform?.join(`\n`)})
    //aUniformDiff.myText(`${nRewrite}`).prop({title:StrMakeUniform?.join(`\n`)})
    aReserved.myText(`${nReserved}`).prop({title:StrReserved?.join(`\n`)})
    aUniformDiff.myText(`-`).prop({title:undefined})
    setMess(strLab+', Check: Done'); blanket.hide();
  });

  var smMultWork
  var strLab='SM multiples'
  var head=createElement('h3').css({'text-align':'left'}).myAppend(strLab).prop({title:'... as read from the db-file'});
  ...
  var labTRes=createElement('label').myText(`TRes: `).prop({title:'mod-time-resolution (independant of what is stored in the settings)'})
  var divTRes=createElement('div').myAppend(labTRes, selTRes)


  var headB0=createElement('dt').css({'text-align':'left'}).myAppend("Find sm multiples"); //.prop({title:'Check for hashcode uniformity'})
  
  var spanLab=createElement('span').myText('Uniform hash: ').prop({title:`... having the same hash code (so nothing will be done to these files)`})
  var funUniform=makeOpenExtCB(PathSMMult.smMultHashUniform);
  var funMakeUniform=makeOpenExtCB([pathMTimeRewrite]);
  var aUniform=createElement('a').prop({href:''}).myAppend(`-`).on('click',funUniform);
  var divSMMultHashUniform=createElement('div').myAppend(spanLab, aUniform)

  var spanLab=createElement('span').myText('Non-uniform hash: ').prop({title:`... at least one of the files having a different hash code.`})
  var fun=makeOpenExtCB(PathSMMult.smMultHashNonUniform);
  var aNonUniform=createElement('a').prop({href:''}).myAppend(`-`).on('click',fun);
  var divSMMultHashNonUniform=createElement('div').myAppend(spanLab, aNonUniform)
  var dd0=createElement('dd').myAppend(butCheck, divSMMultHashUniform, divSMMultHashNonUniform)
  
  var headB1=createElement('dt').css({'text-align':'left'}).myAppend("Don't use these sm").prop({title:`In this "input"-file you can write (copy-paste) sm combinations that you don't want to use.\nSM combinations in this file are not available when new sm are to be assigned (When clicking "Find new mtimes" below).\nThe formating of the file is such as in the db-file (So copy and paste from a db-file (including the two first rows))`})
  var aReserved=createElement('a').prop({href:''}).myAppend(`-`).css({display:'block'}).on('click', async function(ev){
    ev.preventDefault()
    var [err]=await myTouch(pathSMReserved.fsName); if(err) {debugger; myConsole.error(err); resetMess();  return;}
    var f=makeOpenExtCB(pathSMReserved);
    f.call(this, ev)
  });
  var dd1=createElement('dd').myAppend(aReserved)


  var headB2=createElement('dt').css({'text-align':'left'}).myAppend("Find new mtimes").prop({title:'Find new mtimes so that sm is unique for unique hashcodes'})
  var buttCalcTDiff=createElement('button').myAppend(`Find`).css({'margin-right':'.4em'}).on('click', async function(){
    var strLab=`Calc tDiff`
    myConsole.clear(); setMess(strLab+' ...'); blanket.show();
    var [err]=await myTouch(pathSMReserved.fsName); if(err) {debugger; myConsole.error(err); resetMess();  return;}
    var [err, result]=await smMultWork.calcTDiff(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {StrMakeUniform, nRewrite}=result
    aUniformDiff.myText(`${nRewrite}`).prop({title:StrMakeUniform?.join(`\n`)})
    setMess(strLab+': Done'); blanket.hide();
  });
  var fun=makeOpenExtCB(pathMTimeRewrite);
  var aUniformDiff=createElement('a').prop({href:''}).myAppend(`-`).on('click',fun);
  var dd2=createElement('dd').myAppend(buttCalcTDiff, aUniformDiff)


  var headB3=createElement('dt').css({'text-align':'left'}).myAppend("Write new mtimes").prop({title:'... write to files as well as db.'})
  var butMakeUnifrom=createElement('button').myText('Write').on('click', async function(){
    var strLab=`Changing mtimes`
    myConsole.clear(); setMess(strLab+' ...'); blanket.show();
    var [err]=await smMultWork.mySetMTime(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    setMess(strLab+': Done'); blanket.hide();
  });
  var dd3=createElement('dd').myAppend(butMakeUnifrom)

  
  el.myAppend(head, divSide, divTRes, headB0, dd0, headB1, dd1, headB2, dd2, headB3, dd3).css({'text-align':'left'}); // divConsoleT2D, divConsoleT2T, divConsole
...
gThis.viewExtraCreator=function(el){
  ...
  var miniViewSMMatch=miniViewSMMatchCreator(createElement('div'))
  ...
  var divMid=createElement('div').myAppend(miniViewFilterMethodTester, miniViewHashMatch, miniViewSMMatch, miniViewEmptyFolder)
