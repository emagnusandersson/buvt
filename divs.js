...
gThis.miniViewHashMatchCreator=function(el, charSide='S'){
  ...
  var butCheck=createElement('button').myAppend('Check').attr({'title':`... written to ${leafHashMultSMNonUniform}`}).on('click',  async function(){
    myConsole.clear(); setMess(strLab+' ...'); blanket.show();
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, charTResS }=result;
    var arg=extend({}, {fiSourceDir, charTRes:charTResS})
    var [err, nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform]=await getHashMultSMNonUniform(arg); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}

    //var {n, nPatSM, nPatSMMult, nSMMult}=objSM
    aNonUniform.myText(`${nPatNonUniform} (${nTotNonUniform})`)
    aUniform.myText(`${nPatUniform} (${nTotUniform})`)
  ...
  var aUniform=createElement('a').prop({href:''}).myAppend(`-`).prop({title:`patterns (files)\n${fsSMUniform}`}).on('click',funUniform);
  ...
  var aNonUniform=createElement('a').prop({href:''}).myAppend(`-`).prop({title:`patterns (files)\n${fsSMNonUniform}`}).on('click',funNonUniform);
  ...
  var head=createElement('h3').css({'text-align':'left'}).myAppend(strLab); //.prop({title:'... as read from the db ...'})
...
gThis.miniViewSMMatchCreator=function(el, charSide='S'){
  ...
  var butCheck=createElement('button').myAppend('Find').attr({'title':`... written to temporary files`}).on('click', async function(){
    var boTarget=cbSide.checked
    ...
    var {StrNonUniform, StrUniform, nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform}=result; //, StrMakeUniform, nRewrite
  ...
  var head=createElement('h3').css({'text-align':'left'}).myAppend(strLab);
  ...
  var divTRes=createElement('div').myAppend(`TRes: `, selTRes)
  ...
  var dd0=createElement('dd').myAppend(divSide, divTRes, butCheck, divSMMultHashUniform, divSMMultHashNonUniform)
  

  var headB1=createElement('dt').css({'text-align':'left'}).myAppend("Find new mtimes").prop({title:'Find new mtimes so that sm is unique for unique hashcodes'})
  var aReserved=createElement('a').prop({href:''}).myAppend(`Reserved SM`).css({display:'block'}).prop({title:`In this file you can paste reserved sm, formated such as in the db-file`}).on('click', async function(ev){
    ev.preventDefault()
  ...
  var dd1=createElement('dd').myAppend(aReserved, buttCalcTDiff, aUniformDiff)


  var headB2=createElement('dt').css({'text-align':'left'}).myAppend("Write new mtimes").prop({title:'... write to files as well as db.'})
  ...
  var dd2=createElement('dd').myAppend(butMakeUnifrom)

  


  
  el.myAppend(head, headB0, dd0, headB1, dd1, headB2, dd2).css({'text-align':'left'}); // divConsoleT2D, divConsoleT2T, divConsole
