...
gThis.divTopCreator=function(el){
  ...
  el.setButTab=function(strView){
    ...
    var Views=[viewFront, viewCheck, viewCollision, viewExtra, argumentTab];
  ...
  var ButTab=[butFront, butCheck, butCollision, butExtra, argumentButton]
...
gThis.miniViewSMMatchCreator=function(el, charSide='S', boTab=false){
  var butFind=createElement('button').myAppend('Find').attr({'title':undefined}).on('click', async function(){
    ...
    var {StrNonConsistent, StrConsistent, nTotNonConsistent, nPatNonConsistent, nTotConsistent, nPatConsistent}=result; //, nSMSkipList, StrSMSkipList
    
    aConsistent.myText(`${nPatConsistent} (${nTotConsistent})`).prop({title:StrConsistent?.join(`\n`)})
    aNonConsistent.myText(`${nPatNonConsistent} (${nTotNonConsistent})`).prop({title:StrNonConsistent?.join(`\n`)})
  ...
  var smMultWork
  var strHeadA=`Fix SM collisions`, strHeadB=boTab?` (${charSide})`:'', strHead=strHeadA+strHeadB
  var head=createElement('h3').css({'text-align':'left'}).myAppend(strHead).prop({title:'... as read from the db-file'}).css({margin:'0'});
  ...
  var spanLab=createElement('span').myText('Consistent hash: ').prop({title:`... having the same hash code (so nothing will be done to these files)`})
  ...
  var spanLab=createElement('span').myText('Non-consistent hash: ').prop({title:`... at least one of the files having a different hash code.`})
  ...
  var buttFindNewMTime=createElement('button').myAppend(`Find`).css({'margin-right':'.4em'}).on('click', async function(){
    ...
    var {nToChange, StrToChange, nRevert, StrRevert}=result
    aToChange.myText(`${nToChange}`).prop({title:StrToChange?.join(`\n`)})
    aRevert.myText(`${nRevert}`).prop({title:StrRevert?.join(`\n`)})
    setMess(strLab+': Done'); blanket.hide();
  });
  var spanLab=createElement('span').css({'text-align':'left', 'font-weight':'bold'}).myAppend("Find new mtimes").prop({title:`Find new mtimes so that sm is unique for unique hashcodes`})
  var headFindNewMTime=createElement('div').css({'text-align':'left'}).myAppend(spanLab, ' ', buttFindNewMTime)

    // MakeUnifrom
  var spanLab=createElement('span').css({'text-align':'left', 'font-weight':'bold'}).myAppend("Write new mtimes").prop({title:'... to files as well as db.'})
...
gThis.miniViewHashMatchCreator=function(el, charSide='S', boTab=false){
  ...
  var butFind=createElement('button').myAppend('Find').attr({'title':undefined}).on('click',  async function(){
    ...
    var {nTotNonConsistent, nPatNonConsistent, nTotConsistent, nPatConsistent, StrNonConsistent, StrConsistent, nToChange, nRevert, StrToChange, StrRevert}=result
  ...
  var strHeadA=`Fix hash collisions`, strHeadB=boTab?` (${charSide})`:'', strHead=strHeadA+strHeadB
...
gThis.viewExtraCreator=function(el){
  ...
  var divMid=createElement('div').myAppend(miniViewFilterMethodTester, miniViewEmptyFolder)
  .css({display:"flex", gap: "10px", margin:"3px 0px", "flex-direction":"row", "flex-wrap":"wrap"})
