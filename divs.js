...
gThis.divTopCreator=function(el){
  ...
  el.setUIBasedOnSetting=async function(){
    ...
    linkTDb.prop({title:strTargetDbDir}).toggleClass('disabled', boRemote); 
...
gThis.viewFrontCreator=function(el){
  ...
  el.divT2TUsingHash.css({width:'min-content'})
  ...
  el.divExtraTargetF=createElement('textarea').attr({disabled:true}).css({'vertical-align':'middle'})  //.attr({readonly:true})
  var divTextArea=createElement('div').myAppend('Extra Target folders: ', el.divExtraTargetF).css({'grid-area':'1/1/span 1/span 2'})
  ...
  //gThis.divMid=
  var divMid=createElement('div').myAppend(divT2DS, divT2DT, el.divT2TUsingHash, divExtra, divCollisionS, divCollisionT).css({display:'grid', 'grid-template-columns':'1fr 1fr auto', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'0 0 auto', 'overflow-y':'auto'}); // , 'grid-template-rows':'auto auto'
