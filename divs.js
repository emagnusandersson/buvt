...
gThis.divTopCreator=function(el){
  ...
  el.setUIBasedOnSetting=async function(){
    ...
    else { var {label, fiSourceDir, fiTargetDbDir, flTargetDataDir, charFilterMethod, suffixFilterFirstT2T, strHostTarget, boRemoteTarget, boAllowLinks, boAllowCaseCollision, strTargetCharSet}=result; } //, boIncIdS, boIncIdT
...
gThis.viewFrontCreator=function(el){
  ...
  el.setBottomMargin=function(hNew){
    ...
    divCollisionSMW.css({'margin-bottom':hTmp+'px'})
  }
  el.clearUI=function(){ 
  ...
  el.setUIBasedOnSetting=async function(){
    ...
    else { var {label, fiSourceDir, fiTargetDbDir, charFilterMethod, suffixFilterFirstT2T, strHostTarget, boRemoteTarget, boAllowLinks, boAllowCaseCollision, strTargetCharSet}=result; } //, boIncIdS, boIncIdT
    //divT2DS.divTabI.toggle(boIncIdS);  divT2DS.divTab.toggle(!boIncIdS)
    //divT2DT.divTabI.toggle(boIncIdT);  divT2DT.divTab.toggle(!boIncIdT)
    ...
    var title=fiSourceDir+charF+leafFilter;   divT2DS.linkFilterFirstT2D.prop({title}).myText(leafFilter)
      ...
      // Set T_- Db link
    var title=strTargetDbDir+charF+leafDb;   divT2DT.linkDb.prop({title}).myText(leafDb).toggleClass('disabled', boRemoteTarget)
      // Set T_- filter link
    var title=strTargetDbDir+charF+leafFilter;   divT2DT.linkFilterFirstT2D.prop({title}).myText(leafFilter).toggleClass('disabled', boRemoteTarget)
  ...
  var divCollisionSMW=divCollisionSMWCreator(createElement('div')).css({'grid-area':'2/1/span 1/span 2'})

  //gThis.divMid=
  var divMid=createElement('div').myAppend(divT2DS, divT2DT, el.divT2TUsingHash, divCollisionSMW).css({display:'grid', 'grid-template-columns':'1fr 1fr auto', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'0 0 auto', 'overflow-y':'auto'}); // , 'grid-template-rows':'auto auto'
...
gThis.viewCheckCreator=function(el){
  ...
  var butCheckViaPythonT=createElement('button').myAppend('Target').on('click', async function(){
    ...
    //viewCheck.butCheckCancel.show(); viewCheck.boCheckCancel=false
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiTargetDbDir, charTResT, strHostTarget:strHost}=result
    var arg=extend({}, {fiDir:fiTargetDbDir, iStart:Number(inpIStart.value), myConsole, charTRes:charTResT, strHost})
    var [err, mess=""]=await checkViaPython(arg); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    setMess(mess); blanket.hide(); viewCheck.butCheckCancel.hide();
...
gThis.miniViewHashMatchDeleteCreator=function(el, charSide='S', boTab=false){
  ...
  var butFind=createElement('button').myAppend('Find').attr({'title':undefined}).on('click',  async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDbDir, flTargetDataDir, strHostTarget, boRemoteTarget }=result;
    ...
    var arg=extend({}, {fiSourceDir, fiTargetDbDir, flTargetDataDir, charTRes, strHostTarget, boRemoteTarget, charSide})
