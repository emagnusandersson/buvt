...
gThis.divTopCreator=function(el){
  ...
  el.mySetLinkNLabel=async function(){
    ...
    if(suffixFilterFirstT2T) divT2T.linkFilterFirstT2T.removeClass('disabled'); else divT2T.linkFilterFirstT2T.addClass('disabled')
      divT2TUsingHash.linkFilterFirstT2T.prop({title:fsFilterFirstT2T})
    if(suffixFilterFirstT2T) divT2TUsingHash.linkFilterFirstT2T.removeClass('disabled'); else divT2TUsingHash.linkFilterFirstT2T.addClass('disabled')
...
gThis.divT2DTabCreator=function(el, boSource=true){
  var self=el
  el.clearVal=function(){
...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  butDoAction.on('click', async function(){
    ...
    var {fiSourceDir, fiTargetDir, fiTargetDbDir, charTResT2D, charFilterMethod, flPrepend, strHostTarget:strHost, boRemote}=result
    var fiDir=boSource?fiSourceDir:fiTargetDir
    var fiDbDir=boSource?fiSourceDir:(fiTargetDbDir||fiTargetDir)
    if(boSource) {strHost="localhost"; boRemote=false;}
    ...
    var arg={leafFilter, fsDir, fsDbDir, charTRes:charTResT2D, charFilterMethod, flPrepend, strHost, boRemote}
...
gThis.divT2DCreator=function(el, boSource=true){
  ...
  var funT2D=async function(){
    ...
    var fiDir=boSource?fiSourceDir:fiTargetDir
    var fiDbDir=boSource?fiSourceDir:(fiTargetDbDir||fiTargetDir)
    var strHost=boSource?"localhost":strHost
    var leafFilter=LeafFilter[charFilterMethod];
    var [err, fsDir]=await myRealPath(fiDir); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var [err, fsDbDir]=await myRealPath(fiDbDir); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var arg={leafFilter, fsDir, fsDbDir, charTRes:charTResT2D, charFilterMethod, flPrepend, strHost, boRemote}
  ...
  var divCategoryTab=el.divCategoryTab=divT2DTabCreator(createElement('div'), boSource)
...
gThis.divT2TCreator=function(el){
  ...
  var funT2T=async function(){
    ...
    myConsole.clear(); el.clearVal(); setMess(strMess); blanket.show();
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDir, fiTargetDbDir, flPrepend, strHostTarget, boRemote, charTResT2D, charTResT2T, charFilterMethod, suffixFilterFirstT2T}=result
    if(!fiTargetDbDir) fiTargetDbDir=fiTargetDir
    ...
    var arg={fiSourceDir, fiTargetDir, fiTargetDbDir, flPrepend, strHostTarget, boRemote, leafFilter, leafFilterFirst, charTRes, charFilterMethod};
  ...
  var butSyncT2TBrutal=createElement('button').myAppend('Sync (no SC for renamed)').attr({'title':'Sync wo renaming\nFiles with the same name, size \nand mod-time are left untouched.\nOthers are overwritten'}).on('click', async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var arg=copySome({}, result, ["fiSourceDir", "fiTargetDir", "fiTargetDbDir", "charTResT2D", "charTResT2T", "strHostTarget", "boRemote"])
  ...
  var divButton=createElement('div').myAppend(hT2T, butCompareT2T, butSyncT2T, el.linkFilterFirstT2T, butSyncT2TBrutal).css({display:'flex', 'align-items':'center', 'column-gap':'3px', background:'var(--bg-color)', flex:"0 1", border:"solid 1px", 'flex-wrap':'wrap'}); //, butWriteMTimeOnSource , position:'sticky'
...
gThis.divT2TTabUsingHashCreator=function(el){
  ...
  el.clearVal=function(){
    ...
    aMoveOnTarget.myText('-').prop({title});  aMoveOnTarget.myText('-').prop({title}); aDeleted.myText('-').prop({title})
  ...
  el.setVal=function(syncT2T){
    ...
    var nT=nT1+nT2
    var boDisable=nCopyToTarget+nRename+nT1+nT2+nDelete==0

    var {strHovCopyToTarget, strHovActionDelete, strHovCopyOnTarget1, strHovCopyOnTarget2, strHovMoveOnTarget}=StrHov

    butDeleteNWrite.prop({disabled:boDisable})
    var title=undefined;
    aCopyToTarget.myText(nCopyToTarget).prop({title:strHovCopyToTarget});
    aCopyOnTarget1.myText(nT1).prop({title:strHovCopyOnTarget1});  aCopyOnTarget2.myText(nT2).prop({title:strHovCopyOnTarget2})
    aMoveOnTarget.myText(nRename).prop({title:strHovMoveOnTarget});  aDeleted.myText(nDelete).prop({title:strHovActionDelete})
...
//<button>Delete</button> <button>Create</button>   <br/>(read fr files)
  ...
  var arrTR=[...tBody.children]
  var [trCopyToTarget, trCopyOnTarget1, trCopyOnTarget2, trMoveOnTarget, trDeleted, trUnchanged, trDeleteNWrite]=arrTR; //trMat1, 
  ...
  var [aDeleted]=trDeleted.querySelectorAll('a')
  var [spanUnchanged]=trUnchanged.querySelectorAll('a,span')
  var [butDeleteNWrite]=trDeleteNWrite.querySelectorAll('button'); 

  aCopyToTarget.on('click', makeOpenExtCB("T2T_copyToTarget"))
  aCopyOnTarget1.on('click', makeOpenExtCB("T2T_copyOnTarget1"))
  aCopyOnTarget2.on('click', makeOpenExtCB("T2T_copyOnTarget2"))
  aMoveOnTarget.on('click', makeOpenExtCB("T2T_moveOnTarget"))
  aDeleted.on('click', makeOpenExtCB("T2T_deleted"))
...
gThis.viewFrontCreator=function(el){
  ...
  el.clearColumnDivsResult=function(){ 
    ...
    divT2D.clearVal(); divT2DT.clearVal(); divT2T.clearVal(); divT2TUsingHash.clearVal();
  }
  ...
  var divMid=createElement('div').myAppend(divT2D, divT2DT, divT2T, divT2TUsingHash).css({display:'grid', 'grid-template-columns':'1fr 1fr 1fr 1fr', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'1 1 auto', 'overflow-y':'auto'}); //.addClass('unselectable')
...
gThis.viewCheckCreator=function(el){
  ...
  var butCheckViaPythonT=createElement('button').myAppend('Target').on('click', async function(){
    ...
    //viewCheck.butCheckCancel.show(); viewCheck.boCheckCancel=false
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiTargetDir, fiTargetDbDir, charTResT2T, strHostTarget:strHost, boRemote}=result
    var arg=extend({}, {fiDir:fiTargetDir, fiTargetDbDir, iStart:Number(inpIStart.value), myConsole, charTRes:charTResT2T, strHost, boRemote})
    var [err, mess=""]=await checkViaPython(arg); if(err) {debugger; myConsole.error(err); }
    setMess(mess); blanket.hide(); viewCheck.butCheckCancel.hide();
  });
...
gThis.argumentTabExtend=function(el){
  ...
  el.getSelectedFrFile=async function(){
    ...
    extend(objOptExtended, {strHostTarget, boRemote})
