...
gThis.divTopCreator=function(el){
  ...
  el.setUIBasedOnSetting=async function(){
    ...
    else { var {label, fiSourceDir, fiTargetDir, fiTargetDbDir, charFilterMethod, suffixFilterFirstT2T, strHostTarget, boRemote, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strFsTarget, strExtraTargetFolders}=result; }
...
gThis.viewFrontCreator=function(el){
  ...
  el.setBottomMargin=function(hNew){
    ...
    //divMid.css({'margin-bottom':hTmp+'px'})
  ...
  el.setUIBasedOnSetting=async function(){
    ...
    divT2DS.divTabI.toggle(boIncIdS);  divT2DS.divTab.toggle(!boIncIdS)
    divT2DT.divTabI.toggle(boIncIdT);  divT2DT.divTab.toggle(!boIncIdT)
    textareaExtraTargetFolders.myText(strExtraTargetFolders) //+' ◂'
  ...
  var divT2DS=divT2DCreator(createElement('div'), 'S')
  var divT2DT=divT2DCreator(createElement('div'), 'T')
  //gThis.divT2T=divT2TCreator(createElement('div'))
  var divT2TUsingHash=divT2TUsingHashCreator(createElement('div'))
  divT2TUsingHash.css({width:'min-content'})


  var charSide='S'
  var miniViewSMMatchS=miniViewSMMatchCreator(createElement('div'), charSide)
  var miniViewHashMatchS=miniViewHashMatchCreator(createElement('div'), charSide)
  var divCollisionS=createElement('div').myAppend(miniViewSMMatchS, miniViewHashMatchS).css({'grid-area':'3/1'})
  divCollisionS.css({display:'grid', 'grid-template-columns':'1fr 1fr', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'1 1 auto'});
  

  var textareaExtraTargetFolders=createElement('textarea').attr({disabled:true}).css({'vertical-align':'middle'})  //.attr({readonly:true})
  var divTextArea=createElement('div').myAppend('Extra Target folders: ', textareaExtraTargetFolders).css({'grid-area':'1/1/span 1/span 2'})

  var charSide='T'
  var miniViewSMMatchT=miniViewSMMatchCreator(createElement('div'), charSide)
  var miniViewHashMatchT=miniViewHashMatchCreator(createElement('div'), charSide)
  var divCollisionT=createElement('div').myAppend(divTextArea, miniViewSMMatchT, miniViewHashMatchT).css({'grid-area':'3/2'});
  divCollisionT.css({display:'grid', 'grid-template-columns':'1fr 1fr', 'grid-template-rows':'auto auto', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'1 1 auto'});  //, 'overflow-y':'auto'

  var divExtra=createElement('h3').myAppend('Collision checks').css({margin:0, 'grid-area':'2/1/span 1/span 2'})
  //gThis.divMid=
  var divMid=createElement('div').myAppend(divT2DS, divT2DT, divT2TUsingHash, divExtra, divCollisionS, divCollisionT).css({display:'grid', 'grid-template-columns':'1fr 1fr auto', 'column-gap':'3px','margin-top':'0px', 'margin-bottom':'0px', flex:'0 0 auto', 'overflow-y':'auto'}); // , 'grid-template-rows':'auto auto'
...
gThis.miniViewSMMatchCreator=function(el, charSide='S', boTab=false){
  ...
  var butFind=createElement('button').myAppend('Find').attr({'title':undefined}).on('click', async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDir, fiTargetDbDir, strHostTarget, boRemote, strExtraTargetFolders }=result;
    ...
    var arg=extend({}, {fiSourceDir, fiTargetDir, fiTargetDbDir, charTRes, strHostTarget, boRemote, charSide, strExtraTargetFolders})
...
gThis.ArgumentTabRow={
  ...
  mySetDomRow:function(rowData){
    var {StrColOrder}=this
    var {boSelected, label, fiSourceDir, fiTargetDir, fiTargetDbDir, suffixFilterFirstT2T, charTResS, charTResT, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strFsTarget, strExtraTargetFolders}=rowData; //, leafFilterFirst, leafDb
    var elR=this;
...
ms: all characters except ""*/:<>?\\|" (microsoft (ntfs, exfat (fat32 with vfat)))`,
  strExtraTargetFolders:`If the target-side is for example a loose disk or a remote server, and the target-db contains multiple target-folders (such as backups from other computers).
This info is used when assigning new timestamps on the target side.
The target-folders ("strExtraTargetFolders" + "fiTargetDir" (declared above)) will NOT have their timestamp changed (though their timestamps will be read to prevent that new timestamps collides)`
}
...
gThis.argumentSetPopExtend=function(el){
  ...
  var save=async function(){ 
    ...
    var strExtraTargetFolders=textareaExtraTargetFolders.value
    var rT={label, fiSourceDir, fiTargetDir, fiTargetDbDir, suffixFilterFirstT2T, charTResS, charTResT, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strFsTarget, strExtraTargetFolders}; //, leafFilterFirst, leafDb
  ...
  el.setUp=function(){
    ...
    textareaExtraTargetFolders.value=rowDataLoc.strExtraTargetFolders
    inpLabel.focus();
    myConsole.clear();
  ...
  var labAllowCaseCollision=createElement('label').myAppend('Allow case-collisions').prop({title:StrHeaderTitle.boAllowCaseCollision});
  var labFsTarget=createElement('label').myAppend('FsTarget').prop({title:StrHeaderTitle.strFsTarget});
  var labExtraTargetFolders=createElement('label').myAppend('ExtraTargetFolders').prop({title:StrHeaderTitle.strExtraTargetFolders});
  ...
  var textareaExtraTargetFolders=createElement('textarea');
  ...
  var Lab=[labLabel, labSourceDir, labTargetDir, labTargetDbDir, labFilterFirstT2T, labTResS, labTResT, labHostTarget, labFilterMethod, labIncIdS, labIncIdT, labAllowLinks, labAllowCaseCollision, labFsTarget, labExtraTargetFolders];
  var Inp=[inpLabel, inpSourceDir, inpTargetDir, inpTargetDbDir, inpFilterFirstT2T, selTResS, selTResT, inpHostTarget, selFilterMethod, inpIncIdS, inpIncIdT, inpAllowLinks, inpAllowCaseCollision, selFsTarget, textareaExtraTargetFolders];
  ...
  var arrT=[inpSourceDir, inpTargetDir, inpTargetDbDir, textareaExtraTargetFolders]; arrT.forEach(ele=>ele.css({'margin-left':'0em', width:'-webkit-fill-available'}))
...
gThis.argumentDeletePopExtend=function(el){
  ...
  //label, fiSourceDir, fiTargetDir, fiTargetDbDir, leafFilterFirst, suffixFilterFirstT2T, leafDb, charTResS, charTResT, strHostTarget, charFilterMethod, boIncIdS, boIncIdT, boAllowLinks, boAllowCaseCollision, strFsTarget, strExtraTargetFolders
