...
CopyOnTarget: ${nT} file${pluralS(nT)}`
  ...
  async makeChanges(){
    var {charTRes, leafFilter, leafFilterFirst, categoryDelete, copyOnTarget1, moveOnTarget, moveOnTargetNSetMTime, categorySetMTime, copyToTarget, copyOnTarget2, arrDbS, arrDbTNonRelevant, fsSourceDir, fsTargetDbDir, fsTargetDataDir, fleData, arrSource, strHostTarget, charFilterMethod}=this;
    ...
    if(rtrim(fleData).length>0){
      for(var row of arrSource) row.strName=fleData+row.strName
    }
