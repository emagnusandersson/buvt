...
class CategoryDelete{ 
  constructor(arg){
    ...
    copySome(this, arg, ["RelationHash", "strHostTarget", "fsSourceDir", "fsTargetDataDir"]);
  ...
  async delete(){
    var {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir, arrEntry}=this
...
var CopyProt={
  getId:async function(){
    var {arrEntry, fsTargetDataDir, strHostTarget}=this, n=arrEntry.length
...
var CopyToTarget={ // For dealing with all files that needs to be copied TO the target (such as via a slow network)
  factory(arg){
    var that={}
    var {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir}=arg;
    //copySome(that, arg, ["strHostTarget"]); //"fiSourceDir", "fsTargetDataDir", "fiTargetDbDir", , "charFilterMethod"
    extend(that, {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir}); //charTRes, leafFilter, leafFilterFirst
  ...
  // async renameToFinal(){
  //   var {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir, arrEntry, fsTargetTmpDir}=this
  //   //var {fsTmpTarget}=gThis
...
var CopyOnTarget2={
  ...
  async copy(){
    var {strHostTarget, fsSourceDir, fsTargetDataDir, arrPairCopy}=this
...
var CopyOnTarget1={
  ...
  async copyToTemp(){
    var {strHostTarget, fsSourceDir, fsTargetDataDir, arrPairCopy, arrPairFinal}=this
    var [err, Id]=await copyLocally(fsTargetDataDir, arrPairCopy, strHostTarget);  if(err) {debugger; return [err];}
    if(Id.length!=arrPairCopy.length) {debugger; return [Error("Id.length!=arrPairCopy.length")]}
    var arrCouldNotCopy=[], n=arrPairCopy.length
    for(var i=0;i<n;i++){
  ...
  async renameToFinal(){
    var {strHostTarget, fsSourceDir, fsTargetDataDir, FsNew, FsTmp}=this
...
class MoveOnTargetProt{
  constructor(arg, boWhenMTimeIsMatching){
    ...
    extend(this, {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir}); //charTRes, leafFilter, leafFilterFirst
  }
  async renameToTemp(){
    var {RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir, arrRename, FsOld, FsNew, FsTmp}=this
...
class ExtraTargetFReaderWriter{
  constructor(arg){
    copySome(this, arg, ["fsTargetDbDir", "fsTargetDataDir", "strHostTarget"]);
  }
  async read(flTargetF){
    var {fsTargetDbDir, fsTargetDataDir, strHostTarget}=this
    ...
    var FlExtraF=FlF.filter(flFt=>flFt!=flTargetF)
    extend(this, {FlF, FlExtraF})
    return [null, {FlF, FlExtraF}]
  }
  mergeIn(flF){
    var {FlF, FlExtraF}=this;
    if(FlF.indexOf(flF)==-1) FlF.push(flF);
  }
  mySet(FlF){ this.FlF=FlF }
  async write(){
    var {fsTargetDbDir, fsTargetDataDir, strHostTarget, FlF, FlExtraF}=this;
...
class SyncT2TUsingHash{
  constructor(arg){
    ...
    copySome(this, arg, ["fsSourceDir", "fsTargetDbDir", "fsTargetDataDir", "strHostTarget", "charFilterMethod", "boAllowLinks", "boAllowCaseCollision", "strTargetCharSet"]);
    extend(this, {charTRes, leafFilter, leafFilterFirst})
  }
  
  async compare(){  //runOps
    var {charTRes, leafFilter, leafFilterFirst, fsSourceDir, fsTargetDbDir, fsTargetDataDir, strHostTarget, charFilterMethod, boAllowLinks, boAllowCaseCollision, strTargetCharSet}=this
    ...
    var flPrepend=calcFlPrepend(fsTargetDbDir, fsTargetDataDir); extend(this, {flPrepend})
    ...
    var arg={charTRes, leafFilter, leafFilterFirst, fsDir:fsTargetDataDir, strHost:strHostTarget, charFilterMethod}
    ...
    var objArg={RelationHash, strHostTarget, fsSourceDir, fsTargetDataDir}
...
CopyOnTarget: ${nT} file${pluralS(nT)}`
  ...
  async makeChanges(){
    var {charTRes, leafFilter, leafFilterFirst, categoryDelete, copyOnTarget1, moveOnTarget, moveOnTargetNSetMTime, categorySetMTime, copyToTarget, copyOnTarget2, arrDbS, arrDbTNonRelevant, fsSourceDir, fsTargetDbDir, fsTargetDataDir, flPrepend, arrSource, strHostTarget, charFilterMethod}=this;
  ...
  async createFolders(){
    var {arrCreateF, fsTargetDataDir, strHostTarget}=this;
  ...
  async deleteFolders(){
    var {arrDeleteF, fsTargetDataDir, strHostTarget}=this;
