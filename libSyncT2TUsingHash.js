...
class SyncT2TUsingHash{
  ...
  async compare(){
    ...
    var StrTmp=formatList(arrSourcefRem, ['strType', 'size', 'mtime_ns64', 'strName']);
    var strShortListS=formatTitleStr(StrTmp);
    var myResultWriter=new MyWriterSingle(gThis.PathSingleS.notInDb, StrTmp);
    ...
    var StrTmp=formatList(arrTargetfRem, ['strType', 'size', 'mtime_ns64', 'strName']);
    var strShortListT=formatTitleStr(StrTmp);
    var myResultWriter=new MyWriterSingle(gThis.PathSingleT.notInDb, StrTmp);
    var [err]=await myResultWriter.writeToFile();  if(err) {debugger; return [err];}

    var boOK=arrTargetfRem.length==0 && arrTargetfRem.length==0
    var objCoverage={boOK, arrSourcefRem, arrTargetfRem, strShortListS, strShortListT}
    var objOut={objCoverage}
    if(!boOK) {
      //debugger; return [Error(`The target db doesn't cover all files, make sure to sync the db first.`)];
      return [null, objOut]
    }
    ...
    var RelationHash=bundleOnProperty2(arrSource, arrTarget, 'strHash'); // row=>row.strHash
    ...
    var catPrim1=new CatPrim()
    catPrim1.assignFromRel(RelationHash);
    extend(this, {catPrim1})
