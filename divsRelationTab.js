...
gThis.RelationTdKey={  
  mySetVal:function(strKey){
    var boExist=relationTab.objKey[strKey], strCol=boExist?"":'var(--text-red)', title=boExist?undefined:'Location does not exist';
    this.firstChild.myText(strKey).css({color:strCol}).prop({title})
  },
  factory:function(){ 
    var span=createElement('span').myHtml('(key)');
    var td=createElement('td').myAppend(span);  copySome(td, RelationTdKey, ['mySetVal']);  return td;
  },
  factoryS:function(){  var td=RelationTdKey.factory(); td.charSide='S'; return td;},
  factoryT:function(){  var td=RelationTdKey.factory(); td.charSide='T'; return td;}
}
...
gThis.RelationRow={
  ...
  connectStuff:function(){
    ...
    var TDConstructors={
      ...
      keySource:RelationTdKey.factoryS,
      keyTarget:RelationTdKey.factoryT
    }
...
gThis.relationTabExtend=function(el){
  ...
  el.getDataFrFile=async function(){
    ...
    for(var i=0;i<ObjOptRelation.length;i++){
      var objOpt=ObjOptRelation[i]; //objOpt.ind=i;
    ...
    if(!Array.isArray(ObjOptLocation))  ObjOptLocation=[];
      ...
      // objKey
    el.Key=[]; el.objKey={}; for(var o of ObjOptLocation){ el.objKey[o.key]=true; el.Key.push(o.key); }
