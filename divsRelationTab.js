...
gThis.relationSetPopExtend=function(el){
  ...
  el.openFunc=function(boUpdT){
    ...
    if(this==null){rowDataLoc=extend({},RelationRow.propDefaultWInd);}
      ...
      //rowDataLoc=extend({},elR.dataset); // Perhaps one should have a function dataset2Real
...
gThis.relationDeletePopExtend=function(el){
  ...
  el.openFunc=function(){
    ...
    var rowDataLoc=elR.parentNode.wmRowData.get(elR), {idSource, idTarget}=rowDataLoc;
...
gThis.relationTabExtend=function(el){
  ...
  el.myRemove=function(elR){
    ...
    RowCache.push(elR);
    var rowDataLoc=wmRowData.get(elR);
    if(rowDataLoc.boSelected && tBody.childElementCount) {
      //tBody.children[0].dataset.boSelected=true;
      var rowData0=wmRowData.get(tBody.children[0]);
      rowData0.boSelected=true;
    }
    return el; 
  }
  ...
  el.getSelectedFrDom=function(){
    ...
    for(var i=0;i<tBody.childElementCount;i++){
      ...
      // var boSelected=rowA.dataset.boSelected=="true"
      var rowDataLoc=wmRowData.get(rowA), {boSelected}=rowDataLoc;
  ...
  el.clearAllSelectButton=function(){
    ...
    Row.forEach(function(rowA){
      var td=rowA.querySelector('[name=boSelected]'); td.mySetVal(false);
      //rowA.dataset.boSelected=false;
      var rowDataLoc=wmRowData.get(rowA); rowDataLoc.boSelected=false;
    })
  }
  el.setTLastSync=async function(){
    ...
    var rowDataLoc=wmRowData.get(rowSelected);
  ...
  el.getHighestProp=function(key){ //Note! should only be used with properties of type Number.
  ...
  var BoAscDefault={}, Label={boSelected:'Selected', idSource:'Source', idTarget:'Target'}, Th=[];
