...
//categorizeByProp([3,6,9], [2,3,8], x=>x)
//categorizeByProp([3,6,9], [2,3,3,8], x=>x)
//categorizeByProp([1,3,3,6,9], [2,3,3], x=>x)
//categorizeByProp(["aa","progC","qrs"], ["abc", "progC/abc", "progC/def", "progC/ghi", "ss"], funStrShortest)



/***************************************************************************************
 * [arrUniq, objMult, arrUniqified]=extractUniques(arr, arrKey)
 ...
 *   NOTE!!! arr is expected to be sorted (equal entries (with respect to arrKey) next to each other)
 ***************************************************************************************/
...
// var bucketifyByKeyMultSum=function(arrA, arrKey){ // bucketifyByKey Multple Summary
//   var objMult={}, nMult=0
//   var objEntryByKey=bucketifyByKey(arrA, arrKey);
...
// var objEntryByKey=bucketifyByKey(arrA, arrKey),  nPat=Object.keys(objEntryByKey).length
...
var objManyToManyRemoveEmpty=function(objA, objB){  // Modifies objA and objB
  ...
  // objA, objB comes from (is the output of) categorizeByProp.
  // Thus objA, objB has the same property keys.

  // Ex (only tested on python (not js (yet))):
  // Generate the input (objA and objB) using categorizeByProp
  // arrA=[{"a":1,"b":1}, {"a":1,"b":2}, {"a":2}, {"a":2}, {"a":3}]
  // arrB=[{"a":1,"b":1}, {"a":1,"b":3}, {"a":2}, {"a":2}, {"a":4}]
  // objA, objB=categorizeByProp(arrA, arrB, row=>row.a)
  ...
  getN(){
    ...
    for(var i=0;i<3;i++){
      for(var j=0;j<3;j++){
        ...
        NPat[i][j]=nPatT; 
        nPat+=nPatT;
        var nPatAT=nPatT; if(i==0) nPatAT=0;
        var nPatBT=nPatT; if(j==0) nPatBT=0;
        nAPat+=nPatAT
        nBPat+=nPatBT
    ...
    var [nPatA10,nPatY11,nPatY12]=NPat[1]; // Y = A or B
    var [nPatA20,nPatY21,nPatY22]=NPat[2]
    ...
    if(nPatY12!=nA12) {debugger; return [Error("nPatA12!=nA12")];}
    if(nPatY21!=nB21) {debugger; return [Error("nPatA21!=nB21")];}
    ...
    var objDetail={nPatY11,nPatY12,nPatY21,nPatY22, nPatA10,nPatA20,nPatB01,nPatB02,  nA10,nA11,nA12,nA20,nA21,nA22,  nB01,nB02,nB12,nB21,nB22}
