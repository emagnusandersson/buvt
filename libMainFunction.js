...
// var parseStringWExtraTargetFolders=async function(strData){
//   strData=strData.trim();  var StrF=strData.split('\n');
...
class SMMultWork{
  constructor(){ }
  async getMult(arg){
    var {fiSourceDir, fiTargetDir, fiTargetDbDir, charTRes, strHostTarget, boRemote, charSide, strExtraTargetFolders}=arg
    ...
    if(charSide=='S'){ 
      ...
      var strData=strExtraTargetFolders.trim(), FlExtra=strData.split('\n');
      var arrTExtraRem=arrTargetDb;
      for(var i=0;i<FlExtra.length;i++){
        var flTmp=FlExtra[i]
