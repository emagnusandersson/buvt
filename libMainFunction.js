...
class SMMultWork{
  ...
  async getMult(arg){
    ...
    for(var keySM in BundSMMult){
      var arr=BundSMMult[keySM], hash0=arr[0].strHash, boNonConsistentHash=false
      for(var i=1;i<arr.length;i++){
        var {strHash}=arr[i];
        if(hash0!=strHash) { boNonConsistentHash=true; break; }
      }
      if(boNonConsistentHash) { BundSMMultHashNonConsistent[keySM]=arr; nTotNonConsistent+=arr.length;}
      else {BundSMMultHashConsistent[keySM]=arr; nTotConsistent+=arr.length;}
    }
...
class HashMultWork{
  ...
  async getMult(arg){
    ...
    for(var h in BundHashMult){
      var arr=BundHashMult[h], sm0=arr[0].sm, boNonConsistentSM=false
      for(var i=1;i<arr.length;i++){
        var {sm}=arr[i];
        if(sm0!=sm) {boNonConsistentSM=true; break;}
      }
      if(boNonConsistentSM) { BundHashMultSMNonConsistent[h]=arr; nTotNonConsistent+=arr.length;}
      else {BundHashMultSMConsistent[h]=arr; nTotConsistent+=arr.length; }
    }
    ...
    for(var h in BundHashMultSMNonConsistent){
      var arr=BundHashMultSMNonConsistent[h]; obj[h]={};
      for(var i=0;i<arr.length;i++){
        var r=arr[i], {sm}=r;    if(!(sm in obj[h])) obj[h][sm]=[];    obj[h][sm].push(r)
      }
    }

    var BoAnyExtra={}
    for(var kSM in obj){ 
      var BundSM=Object.values(obj[kSM]), boAnyExtra=false;
      ...
      BoAnyExtra[kSM]=boAnyExtra;
    }

    var StrToChange=[], StrRevert=[]
    for(var h in obj){
      var BundSM=Object.values(obj[h]), boAnyExtra=BoAnyExtra[h];
      ...
      var bundSM0=BundSM[0], {nRelevant, nExtra}=bundSM0, r00=bundSM0[0], {size, mtime_ns64, strName}=r00;
      var iStart=boAnyExtra?0:1; // If no extras, then one can leave one (the first) bundSM unchanged
      for(var i=iStart;i<BundSM.length;i++){ 
