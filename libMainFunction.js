...
class SMMultWork{
  constructor(){ }
  async getMult(arg){
    //var {fiDir, fiDbDir, charTRes, strHostTarget, boRemote, fiTargetDir, fiTargetDbDir, charSide}=arg
    ...
    if(charSide=='S'){
      var arrReadOnly=arrTR, arrRelevant=arrSourceDb
    } else {
      //var arrReadOnly=arrTS;
      var arrReadOnly=arrSourceDb;
      var arrRelevant=arrTR
    }
    ...
    var StrReadOnly=formatTitle(StrLongList)
    ...
    for(var k in BundSMMult){
      var arr=BundSMMult[k], hash0=arr[0].strHash, boNonConsistentHash=false
      for(var i=1;i<arr.length;i++){
        var {strHash}=arr[i];
        if(hash0!=strHash) { boNonConsistentHash=true; break; }
      }
      if(boNonConsistentHash) { BundSMMultHashNonConsistent[k]=arr; nTotNonConsistent+=arr.length;}
      else {BundSMMultHashConsistent[k]=arr; nTotConsistent+=arr.length;}
    }
    ...
    var StrConsistent=formatTitle(StrLongList)
    ...
    var StrNonConsistent=formatTitle(StrLongList)
    ...
    return [null, {StrConsistent, nTotConsistent, nPatConsistent, StrNonConsistent, nTotNonConsistent, nPatNonConsistent, nReadOnly, StrReadOnly}]
  }

  async findNewMTime(){
    var {fsDb, fsDbDir, arrRelevant, boRemote, strHostTarget, charTRes, arrAll, charSide}=this

    //var [err, arrTS, arrTR]=await this.getTargetParts(arg); if(err) {debugger; return [err];}
    //var arrReadOnly=arrTR
    ...
    var [err, obj, Arr]=parseRelations(strData, true, undefined, arr=>arr[1]); if(err) return [err]
      // Arr is the same as BundSMMultHashNonConsistent (i think)
    // var fsTmp=pathReadOnly.fsName, [err, strData]=await readStrFile(fsTmp); if(err) return [err]
    // var [err, arrReadOnly]=parseSSVWType(strData); if(err) return [err]
    ...
    for(var s in BundSize){
      var arr=BundSize[s];   BoSizeMTime[s]={}
    ...
    //   var row=arrReadOnly[i], {size, mtime_ns64}=row;
    ...
    var StrToChange=[], nToChange=0, StrRevert=[], BoAnyExtra={}
    for(var k in obj){ 
      var BundHash=Object.values(obj[k]), boAnyExtra=false;
      ...
      BoAnyExtra[k]=boAnyExtra;
    }
    for(var k in obj){
      var boAnyExtra=BoAnyExtra[k]
      var BundHash=Object.values(obj[k]);
      ...
      var iStart=boAnyExtra?0:1
      for(var i=iStart;i<BundHash.length;i++){ // Skipping the first bundHash
    ...
    var StrLongList=StrToChange, StrToChange=formatTitle(StrLongList)
    if(StrLongList.length) StrLongList.unshift(strHead);
    var [err]=await writeFile(gThis[`PathSingle${charSide}`].smToChange.fsName, StrLongList.join('\n')); if(err) {debugger; return [err];}
  
    var StrLongList=StrRevert, StrRevert=formatTitle(StrLongList)
    if(StrLongList.length) StrLongList.unshift(strHead);
    var [err]=await writeFile(gThis[`PathSingle${charSide}`].smRevert.fsName, StrLongList.join('\n')); if(err) {debugger; return [err];}
  
    return [null, {nToChange, StrToChange, nRevert, StrRevert}]
  }
...
class HashMultWork{
  constructor(){ }
  async getMult(arg){
    var {fiDir, fiDbDir, charTRes, strHostTarget, boRemote, charSide}=arg
    ...
    if(err){    if(err.code==STR_ENOENT){err=null; strData=""} else{ debugger; return [err]}    }
    var [err, arrDb]=parseDb(strData, charTRes); if(err) {debugger; return [err];}
    //var myResultWriter=new MyWriter(PathHashMult)
    ...
    for(var k in BundHashMult){
      var arr=BundHashMult[k], sm0=arr[0].sm, boNonConsistentSM=false
      for(var i=1;i<arr.length;i++){
        var {sm}=arr[i];
        if(sm0!=sm) {boNonConsistentSM=true; break;}
      }
      if(boNonConsistentSM) { BundHashMultSMNonConsistent[k]=arr; nTotNonConsistent+=arr.length;}
      else {BundHashMultSMConsistent[k]=arr; nTotConsistent+=arr.length; }
    }
    ...
    var obj={}
    for(var k in BundHashMultSMNonConsistent){
      var arr=BundHashMultSMNonConsistent[k]; obj[k]={};
      for(var i=0;i<arr.length;i++){
        var r=arr[i], {sm}=r;    if(!(sm in obj[k])) obj[k][sm]=[];    obj[k][sm].push(r)
      }
    }

    //var arrRewrite=[]
    var StrToChange=[], StrRevert=[]
    for(var k in obj){ // Looping through all hashes
      var arrArr=Object.values(obj[k])
        // Sort so that longest sm-set is first
      arrArr.sort((A,B)=>{var a=A.length, b=B.length; return (a<b) ? 1 : ((a>b)?-1:0)});
      var r00=arrArr[0][0], {size, mtime_ns64, strName}=r00; 
      for(var i=1;i<arrArr.length;i++){ // Skipping the first (longest) sm-set
          // Assign mtime_ns64 to all files with the same sm
        for(var j=0;j<arrArr[i].length;j++){
          var row=arrArr[i][j], {mtime_ns64:mtime_ns64Old, strName}=row;  
          funSetMTime(row, charTRes, mtime_ns64)
          //arrRewrite.push(row);
          StrToChange.push(`${mtime_ns64} ${strName}`)
          StrRevert.push(`${mtime_ns64Old} ${strName}`)
        } 
      }
    }
    var strHead=`int64 string\nmtime_ns64 strName`;

    var StrLongList=StrToChange, nToChange=StrToChange.length
    var StrToChange=formatTitle(StrLongList)
    if(StrLongList.length) StrLongList.unshift(strHead);
    //var myWriter=new MyWriter([pathHSMToChange]); myWriter.Str[0]=StrLongList
    var myWriter=new MyWriterSingle(gThis[`PathSingle${charSide}`].hsmToChange); myWriter.Str=StrLongList
    var [err]=await myWriter.writeToFile();  if(err) {debugger; return [err];}

    var StrLongList=StrRevert, nRevert=StrRevert.length
    var StrRevert=formatTitle(StrLongList)
    if(StrLongList.length) StrLongList.unshift(strHead);
    //var myWriter=new MyWriter([pathHSMRevert]); myWriter.Str[0]=StrLongList
    var myWriter=new MyWriterSingle(gThis[`PathSingle${charSide}`].hsmRevert); myWriter.Str=StrLongList
    var [err]=await myWriter.writeToFile();  if(err) {debugger; return [err];}

    extend(this, {fsDb, arrDb, fsDbDir, boRemote, strHostTarget, charTRes, charSide})
    return [null, {nTotNonConsistent, nPatNonConsistent, nTotConsistent, nPatConsistent, StrNonConsistent, StrConsistent, nToChange, nRevert, StrToChange, StrRevert}]
  }
...
// touch -m -d '@1716905346.502143507' b0.txt
