...
//var checkingDbIntegrity=async function(arrIn, charSide){
var find_XTM_SMHash=async function(arrIn, charSide){ // divDbRelationTestCreator
  var PathCur=gThis[`Path${charSide}`],   myResultWriter=new MyWriter(PathCur)

  var Bund_XTM_SMHash={}, nMult=0, boOK=true;
  arrIn.sort(funIncHash)
  var BundSM=bundleOnProperty(arrIn, 'sm');
  var [BundSMMult, nMult_, arrSingle]=extractBundlesWMultiples(BundSM);
  for(var k in BundSMMult){
    var bundSMMult=BundSMMult[k], n=bundSMMult.length;
    var row0=bundSMMult[0], rowEnd=bundSMMult[n-1];
    if(row0.strHash!=rowEnd.strHash){ Bund_XTM_SMHash[k]=bundSMMult; nMult+=n; boOK=false; } // Since they are sorted it is enough to check the first and the last
  }
  var nKeyMult=Object.keys(Bund_XTM_SMHash).length;

  var funMatch=s=>`MatchingData ${s.size.myPadStart(10)} ${s.mtime_ns64Floored}`,  funUnique=s=>`  ${s.strHash} ${s.strName}`;
  var Str=formatListBundled(Bund_XTM_SMHash, funMatch, funUnique)
  ...
  return [err, {boOK, strShortList, nMult, nKeyMult}]
}
...
var categorizeSMRelations=async function(arrTreef, arrDb, charSide){
  ...
  var arrCreate=[].concat(catPrim2.arrA[1][0], catPrim2.arrA[2][0]);
  var arrDelete=[].concat(catPrim2.arrB[0][1], catPrim2.arrB[0][2]);
  var arrTrM1T1=[].concat(catPrim2.arrA[1][1]);
  var arrDbM1T1=[].concat(catPrim2.arrB[1][1]);
  ...
  var arrTrMMult=[].concat(catPrim2.arrA[1][2], catPrim2.arrA[2][1], catPrim2.arrA[2][2]); // Only for arrDbOwnNew (also recalculated in divT2DTab.setVal)
  var arrDbMMult=[].concat(catPrim2.arrB[1][2], catPrim2.arrB[2][1], catPrim2.arrB[2][2]); // Not used (recalculated in divT2DTab.setVal)
     // Old Categoryzations
  //    ⎧0      0    0   ⎫     ⎧0 delete delete⎫
  // A: |create 1T1  Mult|  B: |0 1T1    Mult  |
  //    ⎩create Mult Mult⎭     ⎩0 Mult   Mult  ⎭

     // New Categoryzations from 20260609
  //    ⎧0      0    0  ⎫     ⎧0 delete delete⎫
  ...
  //myResultWriter.Str.M1T1Relation=formatRelation(catPrim2.ArrA[1][1], catPrim2.ArrB[1][1], ...ObjKey['M1T1'])
  //myResultWriter.Str.M1T1RelationCustom=formatRelationCustom(catPrim2.ArrA[1][1], catPrim2.ArrB[1][1])
  //debugger
  ...
  var objCategory={catPrim1, boChanged, catPrim2, ObjFeedback, arrTrUntouched, arrTrM1T1, arrTrMMult, arrDbUntouched, arrDbM1T1, arrDbMMult, arrCreate}
...
class SyncT2D{
  static async fun1Prework(arg){
    ...
    if(boDataInTop){ var arrDb_Others=arrDb, arrDb_Own=[];}
    else{
    ...
    if(boDataInTop){ var arrTr_Others=arrTreef, arrTr_Own=[];}
    else{
      var [arrTr_Others, arrTr_Own]=ArrDb.selectWPrefixArr(arrTreef, FleSub);
    }
    var objOwnCounter={nTrOwn:arrTr_Own.length, nTrOthers:arrTr_Others.length, nDbOwn:arrDb_Own.length, nDbOthers:arrDb_Others.length, FleSub, boDataInTop}
    extend(objOut, {arrDb_Others, arrDb_Own, arrTr_Own, objOwnCounter})
    if(arrDb_Others.length!=arrTr_Others.length) {debugger; return [Error('arrDb_Others.length!=arrTr_Others.length')];}

    //var objOwn={nOwnDb:arrDb_Own.length, nOthersDb:arrDb_Others.length, nOwnTr:arrTr_Own.length, nOthersTr:arrTr_Others.length, }

    //arrDb_Others.sort(funIncStrName);   arrTr_Others.sort(funIncStrName)
    //var [err, arrTrUntouched, arrDbUntouched, arrTrTouched, arrDbTouched]=extractMatching(arrTr_Others, arrDb_Others, ['strName']); if(err) {debugger; return [err];} 

      // Db check
    var [err, objDb_XTM_SMHash]=await find_XTM_SMHash(arrDb_Own, charSide); if(err) {debugger; return [err];}
    extend(objOut, {objDb_XTM_SMHash})
    if(!objDb_XTM_SMHash.boOK) { return [null, objOut];}
    ...
    extend(objOut, {boCreated, boChanged, objHL, objDb_XTM_SMHash})
...
class MultWork{
  static async getMult(arg){
    ...
    var myResultWriter=new MyWriterSingle(gThis[`PathSingle${charSide}`].collisionXTM, StrLongList);
    var [err]=await myResultWriter.writeToFile();  if(err) {debugger; return [err];}
