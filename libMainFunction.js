...
class SyncDb{
  ...
  async compare(){
    ...
    //myResultWriter.Str.renamed=Category1T1.formatForFile(arrSourceIM, arrTargetIM)
    myResultWriter.Str.copiedThenRenamedToOrigin=T2DCategoryNM.formatForFile(arrSourceNM, arrTargetNM)
  ...
  async readAction(){
    ...
    var fsTmp=PathCur.copiedThenRenamedToOrigin.fsName
    var [err, strData]=await readStrFile(fsTmp); if(err) return [err]
...
class CopyToTarget extends CopyProt{ // For dealing with all files that needs to be copied TO the target (such as via a slow network)
  constructor(arg){
    super(arg)
    var {RelationHash, strHostTarget, boRemote, fsSourceDir, fsTargetDir}=arg;
    //copySome(this, arg, ["strHostTarget"]); //"fiSourceDir", "fsTargetDir", "fiTargetDbDir", , "charFilterMethod"
    extend(this, {RelationHash, strHostTarget, fsSourceDir, fsTargetDir}); //charTRes, leafFilter, leafFilterFirst
...
class CopyOnTarget1 extends CopyProt{
  constructor(arg){
    super(arg)
    var {RelationHash, strHostTarget, boRemote, fsSourceDir, fsTargetDir}=arg;
    //copySome(this, arg, ["strHostTarget"]); //"fiSourceDir", "fsTargetDir", "fiTargetDbDir", , "charFilterMethod"
    ...
    extend(this, {RelArg, Str, arrEntry})
  }
  createFeedback(){ 
    var {Str, RelArg}=this;
    var lStr=Str.length, StrHov=Str.slice(0,nShortListMax);  if(lStr>nShortListMax) { StrHov.push('⋮');}
...
class CopyOnTarget2 extends CopyProt{
  constructor(arg){
    super(arg)
    var {RelationHash, strHostTarget, boRemote, fsSourceDir, fsTargetDir}=arg;
    //copySome(this, arg, ["strHostTarget"]); //"fiSourceDir", "fsTargetDir", "fiTargetDbDir", , "charFilterMethod"
    ...
    var Str=[], RelArgTmp=[], FsNew=[], FsTmp=[], arrEntry=[];
    for(var key in RelationHash){
      ...
      if(nCopyOn2) {
        ...
        for(var i=lB;i<lA;i++) {
          ...
          FsNew.push(fsNew); FsTmp.push(fsTmp); arrEntry.push(rowT)
        }
        RelArgTmp.push({arrS,arrT})
      }
    }
    extend(this, {RelArgTmp, Str, FsNew, FsTmp, arrEntry});
  }
  createFeedback(){ 
    var {Str, RelArgTmp}=this;
    var lStr=Str.length, StrHov=Str.slice(0,nShortListMax);  if(lStr>nShortListMax) { StrHov.push('⋮');}
    var strHov=lStr?StrHov.join('\n'):undefined;
    var nS=0, nT=0;
...
class SyncT2TUsingHash{
  ...
  async compare(){  //runOps
    ...
    if(arrSourcefRem.length>0) {debugger; return [Error('arrSourcefRem.length>0')];}

    var [err, arrTargetfM, arrDbTM, arrTargetfRem, arrDbTRem]=extractMatching(arrTargetf, arrDbT, ['strName'])
    if(arrTargetfRem.length>0) {debugger; return [Error('arrTargetfRem.length>0')];}
  ...
  format(fiDb){ //fsDir, 
    ...
    extend(this, {myResultWriter, ObjFeedback}); //StrHov, objN
  }
...
CopyOnTarget: ${nT} file${pluralS(nT)}`
  ...
  async makeChanges(){
    ...
    await categoryDelete.delete();   // By this point, the old names (that might be reused), have been cleared.
