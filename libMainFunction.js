...
var hardLinkCheck=async function(args){
  ...
  var [err, arrTreef, arrTreeF] =await treeParser.parseTree(fsDirSource, charTRes, leafFilterFirst); if(err) {debugger; return [err];}
...
var hashMultipleCheck=async function(args){
  var {leafDb=settings.leafDb, charTRes=settings.charTRes, flPrepend="", fiDirSource}=args
  var [err, fsDirSource]=await myRealPath(fiDirSource); if(err) {debugger; return [err];}
...
class DoDCActionsToTree{
  constructor(args){
    var {fiDirSource, fiDirTarget, strHostTarget}=args
  ...
  async makeChanges(){
    var {fsDirSource, fsDirTarget, arrChange, arrCreate, arrDelete, arrSourceMultNoName, arrTargetMultNoName, strHostTarget}=this
...
class SyncTreeToTree{
  constructor(args){
    ...
    copySome(this, args, ["fiDirSource", "fiDirTarget", "strHostTarget"]);
  ...
  async compare(){  //runOps
    var {fsDirSource, fsDirTarget, charTRes, leafFilterFirst, strHostTarget}=this
    ...
    var [err, arrTargetf, arrTargetF] =await treeParser.parseTree(fsDirTarget, charTRes, null, strHostTarget); if(err) {debugger; return [err];}
...
var check=async function(args){ // buCopyUnlessNameSMMatch
  ...
  var [err, mess]=await checkInterior(args);
  return [err, mess]
}
...
class RenameFinishToTree{
  constructor(args){
    ...
    copySome(this, args, ["fiDir", "fsRenameFile", "strHostTarget"])
    extend(this, {boDir})
  }
  async read(){
...
class SyncTreeToTreeBrutal{
  constructor(args){
    copySome(this, args, ["fiDirSource", "fiDirTarget", "charTRes", "leafFilterFirst", "strHostTarget"])
  }
  async constructorPart2(){ // Must be awaited
    var {fiDirSource, fiDirTarget, charTRes, leafFilterFirst, strHostTarget}=this
  ...
  async compare(){ // buCopyUnlessNameSMMatch
    var {fiDirSource, fsDirSource, fiDirTarget, fsDirTarget, charTRes, leafFilterFirst, strHostTarget}=this
    var tStart=unixNow()

      // Quick return if target doesn't exists. (It would have been detected when parsing, but this way one doesn't have to wait.)
    //var [err, stats] = await getStats(fsDirTarget); if(err) return [err];
    var [err, boExist]=await fileExist(fsDirTarget); if(err) {debugger; return [err]; }
    if(!boExist){return [Error(`${fsDirTarget} does not exist`)];}
    ...
    setMess(`Parsing target tree`, null, true)
    var [err, arrTargetf, arrTargetF] =await treeParser.parseTree(fsDirTarget, charTRes, null, strHostTarget); if(err) { return [err];}
...
Creating ${lenSourcef} file${pluralS(lenSourcef)} (and ${lenSourceF} folder${pluralS(lenSourceF)})` // 🗎🗀  file${pluralS(lenTargetf)}.  folder${pluralS(lenSourceF)}  file${pluralS(lenSourcef)} folder${pluralS(lenTargetF)}.
  ...
  async makeChanges(){
    var {fsDirSource, fsDirTarget, arrSourcefRem, arrSourceFRem, arrTargetfRem, arrTargetFRem, strHostTarget}=this
    ...
    var [err]=await myCopyEntries(arrSourcefRem, fsDirSource, fsDirTarget, strHostTarget);  if(err) {debugger; myConsole.error(err); return;}
  }
}
