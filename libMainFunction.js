...
var getHashMultSMNonUniform=async function(arg){
  ...
  var StrTmp=formatMatchingDataWMultSingleDataSet(objHashMultSMNonUniform, funMatch, funUnique)
  myResultWriter.Str.hashMultSMNonUniform=StrTmp
  var funMatch=s=>`MatchingData ${s.size.myPadStart(10)} ${s.mtime_ns64}`,  funUnique=s=>`  ${s.strName}`;
  var StrTmp=formatMatchingDataWMultSingleDataSet(objHashMultSMUniform, funMatch, funUnique)
  myResultWriter.Str.hashMultSMUniform=StrTmp

  var [err]=await myResultWriter.writeToFile();  if(err) {debugger; return [err];}
  return [null, nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform]
}
...
class SMMultWork{
  ...
  async getSMMultHashNonUniform(arg){
    ...
    var StrTmp=formatMatchingDataWMultSingleDataSet(objSMMultHashNonUniform, funMatch, funUnique)
    var StrT=StrTmp.slice(0,nShortListMax); if(StrT.length==nShortListMax) StrT.push('⋮');     var StrNonUniform=StrT
    var strHead=`int int64\nsize mtime_ns64Floored\nstring string\nstrHash strName`;
    StrTmp.unshift(strHead); myResultWriter.Str.smMultHashNonUniform=StrTmp
    var funMatch=s=>`MatchingData ${s.size.myPadStart(10)} ${s.mtime_ns64Floored}`,  funUnique=s=>`  ${s.strName}`;
    var StrTmp=formatMatchingDataWMultSingleDataSet(objSMMultHashUniform, funMatch, funUnique)
    var StrT=StrTmp.slice(0,nShortListMax); if(StrT.length==nShortListMax) StrT.push('⋮');     var StrUniform=StrT
    var strHead=`int int64\nsize mtime_ns64Floored\nstring string\nstrHash strName`;
    StrTmp.unshift(strHead); myResultWriter.Str.smMultHashUniform=StrTmp

    var [err]=await myResultWriter.writeToFile();  if(err) {debugger; return [err];}
    extend(this, {fsDb, arrDb, fsDbDir, boRemote, strHostTarget, charTRes})

    return [null, {StrNonUniform, StrUniform, nTotNonUniform, nPatNonUniform, nTotUniform, nPatUniform}]
  }
  ...
  async calcTDiff(){
    ...
    var [err, objReserved, ArrReserved]=parseRelations(strData, true); if(err) return [err]
    ...
    var StrFeedBack=StrMakeUniform
    StrFeedBack=StrFeedBack.slice(0,nShortListMax); if(StrFeedBack.length==nShortListMax) StrFeedBack.push('⋮');
    var strHead=`int int64\nsize mtime_ns64Floored\nint64 string string\ntDiff_ns strHash strName`;
    StrMakeUniform.unshift(strHead); myResultWriter.Str.mtimeRewrite=StrMakeUniform
    var [err]=await myResultWriter.writeToFile();  if(err) {debugger; return [err];}
  ...
  async mySetMTime(){
    var {fsDb, arrDb, fsDbDir, boRemote, strHostTarget, charTRes}=this; //, arrRewrite
    ...
    for(var i=0;i<arrDbMatch.length;i++){
      var rowDb=arrDbMatch[i], rowNew=arrRewriteMatch[i], {mtime_ns64Floored, tDiff_ns}=rowNew, mtime_ns64=mtime_ns64Floored+tDiff_ns
      extend(rowNew, {mtime_ns64, mtime_ns64Floored:mtime_ns64})
...
class SyncDbI{
  ...
  async compare(){
    ...
    ObjFeedback.objSTMatch2={strHov:StrHov.join('\n'), nFile:undefined}
