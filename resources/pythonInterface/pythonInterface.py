...
settings={"charTRes":charTRes, "boIncludeLinks":boIncludeLinks, "leafFilter":leafFilter}
...
class TreeParser:
  ...
  def getBranch(self, objDir, intLevel):
    ...
    for entry in myList:
      ...
      objEntry={"strName":flName, "id":id, "mtime_ns64":mtime_ns64, "mtime_ns64Round":mtime_ns64Round} #, "mtime":mtime
      if(not boDir):
        ...
        objEntry.update({"size":size, "sm":sm, "strType":strType}) 
  ...
  def parseTree(self, fsDir, charTRes, leafFilterFirst=None): #, boIncludeLeafDb=False
