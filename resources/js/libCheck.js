...
var getHighestMissing=async function(fsDir, strFile){
  ...
  while(1){
    ...
    var [err, result]=await myStatsCache.stats(fsDir+charF+strPar); if(err) return [err]  
...
//var checkSummarizeMissingInterior=async function(fsDir, fsDb, charTRes){
var checkSummarizeMissingInterior=async function(args){
  var {fiDir, leafDb=settings.leafDb, charTRes=settings.charTRes, myConsole}=args
  var [err, fsDir]=await myRealPath(fiDir); if(err) {debugger; myConsole.error(err); return;}
  var fsDb=fsDir+charF+leafDb

  var [err, arrDB]=await parseDb(fsDb, charTRes); if(err) return [err]
  var [err, strSum]=await summarizeMissing(arrDB, fsDir); if(err) return [err]
...
//var checkHash=async function(fsDir, fsDb, charTRes, intStart){
var checkInterior=async function(args){
  var {fiDir='.', leafDb=settings.leafDb, charTRes=settings.charTRes, intStart=0, myConsole}=args
  var [err, fsDir]=await myRealPath(fiDir); if(err) {debugger; myConsole.error(err); return;}
  var fsDb=fsDir+charF+leafDb
  var tStart=unixNow()
  ...
  if(nMisMatchTimeSize) myConsole.printNL(`<b>META (SIZE/MTIME) MISMATCHES!!!</b> (on ${nMisMatchTimeSize} file${pluralS(nMisMatchTimeSize)}) This happens when <b>HASHBERT SYNC</b> wasn't called before checking. (So call <b>HASHBERT SYNC</b> before running <b>HASHBERT CHECK</b>)`)
