...
class TreeParserPython{
  constructor(){ }
  async parseTree(fsDir, charTRes, leafFilterFirst=null, strHost=null){
    var boUseFilter=Boolean(leafFilterFirst)
    if(boUseFilter){
      var fsFilterFirst=fsDir+charF+leafFilterFirst
      var [err, boExist]=await fileExist(fsFilterFirst); if(err) {debugger; return [err]; }
      if(!boExist){return [Error(`${fsFilterFirst} does not exist`)];}
    }
    ...
    if(strHost) {
      ...
      arrCommand.unshift('ssh', strHost)
    }
