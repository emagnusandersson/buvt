...
gThis.getSelectedFrFile=async function(){
  ...
  for(var i=0;i<ObjOptRelation.length;i++){
    var objOptRelationTmp=ObjOptRelation[i], {keyTarget:keyTargetTmp, flTargetDataDir}=objOptRelationTmp;
    var flTargetDataDir=trim(flTargetDataDir, charF)+charF
    if(keyTargetTmp==keySource) FleSSub.push(flTargetDataDir)
    if(keyTargetTmp==keyTarget) FleTSub.push(flTargetDataDir)
  }
  ...
  extend(objOptExtended, {objOptSource, objOptTarget, FleSSub, FleTSub, fiTargetDataDir})
...
gThis.getSelectedFrFileWExtra=async function(){
  ...
  //var {fiSourceDir, strHostTarget, fiTargetDbDir, flTargetDataDir, charTResS, charTResT, charTResCollision}=argGeneral
