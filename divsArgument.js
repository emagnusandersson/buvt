...
gThis.getSelectedFrFile=async function(){
  ...
  if(typeof objOptTarget=='undefined') {debugger; return [Error("typeof objOptTarget=='undefined'")];}
    ...
    // FleSSub, FleTSub
  var FleSSub=[], FleTSub=[], boDataInTopS=false
  for(var i=0;i<ObjOptRelation.length;i++){
    ...
    var fleTargetDataDir=trim(flTargetDataDir, charF)+charF
    if(idTargetTmp==idSource) {if(flTargetDataDir.length==0) boDataInTopS=true; FleSSub.push(fleTargetDataDir); }
    if(idTargetTmp==idTarget) FleTSub.push(fleTargetDataDir)
  }
  ...
  var boDataInTopT=flTargetDataDir.length==0;
  var fleTargetDataDir=boDataInTopT?"":flTargetDataDir+charF;

  var {charFilterMethod}=objOptSource;
  var leafFilter=LeafFilter[charFilterMethod]; extend(objOptSource, {leafFilter, charSide:'S', boTarget:false, boDataInTop:boDataInTopS});

  var {charFilterMethod}=objOptTarget;
  var leafFilter=LeafFilter[charFilterMethod]; extend(objOptTarget, {leafFilter, charSide:'T', boTarget:true, boDataInTop:boDataInTopT});
  ...
  extend(objOptExtended, {boRemoteTarget, strHostTarget, fleTargetDataDir})
...
gThis.calcExtraData=async function(arg){ // Extra calculations, caclulating fs from fi
  ...
  var fsTargetDataDir=fsTargetDbDir;   if(!objOptTarget.boDataInTop) {fsTargetDataDir+=`${charF}${flTargetDataDir}`;} 
  var fsSourceDb=fsSourceDir+charF+settings.leafDb,  fsTargetDb=fsTargetDbDir+charF+settings.leafDb

  extend(objOptSource, {fsDataDir:fsSourceDir, fsDb:fsSourceDb})
  extend(objOptTarget, {fsDataDir:fsTargetDataDir, fsDb:fsTargetDb})

  var argExtra={fsSourceDir, fsTargetDbDir, fsTargetDataDir, fsSourceDb, fsTargetDb}
