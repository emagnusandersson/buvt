...
var myRealPathArr=async function(FiEntry, strHost=null){
...
var myCopyEntries=async function(arrEntry, fsSourceDir, fsTargetDataDir, strHost=null){
...
var writeDbWrapper=async function(arrData, fsDb, strHost=null){
  ...
  // Extract files that starts with flPrepend, remove the flPrepend part and return in arrSelected.
...
var selectFrArrDb=function(arrDb, flPrepend){
  ...
  for(var i=0;i<arrDb.length;i++){
    ...
    if(row.strName.slice(0,nPrepend)!=flPrepend) arrRem.push(rowCopy)
    else{
...
// var [arrDbRelevant, arrDbRem] =selectFrArrDb(arrDb, flPrepend)
