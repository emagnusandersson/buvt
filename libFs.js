...
var writeDbWrapper=async function(arrData, fsDb, strHost=null){
  ...
  // Extract files that starts with fleChild, remove the fleChild part and return in arrSelected.
...
var selectFrArrDb=function(arrDb, fleChild){
  var arrRem=[],  arrSelected=[]
  var nPrepend=fleChild.length
  for(var i=0;i<arrDb.length;i++){
    var row=arrDb[i]
    //var rowCopy=copy.copy(row)
    var rowCopy=extend({},row)
    
    if(row.strName.slice(0,nPrepend)!=fleChild) arrRem.push(rowCopy)
    else{
      ...
      arrSelected.push(rowCopy)
    }
  }
  return [arrSelected, arrRem]
}
// var [arrDbRelevant, arrDbRem] =selectFrArrDb(arrDb, fleChild)
...
var calcFleChild=function(fsPar, fsChild){
  ...
  if(fsPar!=fsChildCrop) throw Error(`fsChild is not in fsPar, fsPar='${fsPar}', fsChild='${fsChild}'`)
  var fleChild=fsChild.slice(nPar)+charF;
  fleChild=ltrim(fleChild, charF); // So empty if fsChild is the same as fsPar
  return fleChild
}
