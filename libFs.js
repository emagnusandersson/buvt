...
var readStrFileWHost=async function(fsFileRemote, strHost='localhost'){
  ...
  if(boRemote){
      // Fetch remote file
    var fsFileTmp=FsResultFile['fileRemoteLocalCopy'];
...
var copyLocally=async function(fsDir, Relation, strHost=null, boViaTmpName=false){
  if(!strHost) strHost='localhost';  var boRemote=strHost!='localhost';
  var Str=[]
  ...
  var strHeadMT=`string`, strHeadM=`trash`,   strHeadUT=`string string string`, strHeadU=`side mtime strName`;
  Str.unshift(strHeadMT, strHeadM, strHeadUT, strHeadU)

  var {fsScriptRemote, fsRemoteArg}=interfacePython
  var [err]=await writeFileRemote(fsRemoteArg, Str.join('\n'), strHost); if(err) {debugger; return [err];}

  // const fsArg=FsResultFile['arg']
  // var [err]=await writeFile(fsArg, Str.join('\n')); if(err) {debugger; return [err];}
  
  // var arrCommand=['scp', '-p', fsArg, strHost+':'+fsRemoteArg]
  // var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
  // if(exitCode) { debugger; return [Error(stdErr)]; };

  var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'copyLocally', "--fiDir", fsDir, "--fiFile", fsRemoteArg]
  var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
  if(exitCode) { debugger; return [Error(stdErr)]; }; 
  return [null]
}
...
var renameFiles=async function(arrRename, strHost=null, boViaTmpName=false){
  ...
  // for(var i=0;i<nDir;i++){ FsDir[i]=fsDir+charF+FlDir[i]; }
  ...
  if(boRemote) {
    ...
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'renameFiles', "--fiFile", fsRemoteArg]
...
var myRmFiles=async function(FsName, strHost=null){ //, fsDir
  ...
  if(boRemote) { 
    var {fsScriptRemote, fsRemoteArg}=interfacePython
    var [err]=await writeFileRemote(fsRemoteArg, FsName.join('\n'), strHost); if(err) {debugger; return [err];}
    //const fsArg=FsResultFile['arg']
    ...
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'rmFiles', "--fiFile", fsRemoteArg]
    var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
    if(exitCode) { debugger; return [Error(stdErr)]; };
...
var myRmFolders=async function(FsName, strHost=null){  
  ...
  if(boRemote) {
    ...
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'rmFolders', "--fiFile", fsRemoteArg]
    var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
    if(exitCode) { debugger; return [Error(stdErr)]; };
...
var myMkFolders=async function(FsName, strHost=null){ //, fsDir
  ...
  if(boRemote) { 
    ...
    // var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
    // if(exitCode) { debugger; return [Error(stdErr)]; };
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'makeFolders', "--fiFile", fsRemoteArg]
    var [exitCode, stdErr, stdOut]=await execMy(arrCommand);   if(stdErr) { debugger; return [stdErr];}
    if(exitCode) { debugger; return [Error(stdErr)]; };
...
var getId=async function(FsName, fsDir, strHost=null){
  if(!strHost) strHost='localhost';  var boRemote=strHost!='localhost';
  var nTot=FsName.length
  if(nTot==0) return [null, []];

  const fsArg=FsResultFile['arg']
  var [err]=await writeFile(fsArg, FsName.join('\n')); if(err) {debugger; return [err];}
  var {fsScriptLocal, fsScriptRemote, fsRemoteArg}=interfacePython
  ...
  if(boRemote) { 
    ...
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'getId', "--fiFile", fsRemoteArg];
...
var calcHashes=async function(arrEntry, fsDir, strHost=null){
  ...
  if(boRemote) { 
    ...
    var arrCommand=['ssh', strHost, 'python', fsScriptRemote, 'calcHashes', "--fiFile", fsRemoteArg];
