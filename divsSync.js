...
gThis.divTabT2DCreator=function(el, charSide){
  ...
  el.setVal=function(syncDb){
    ...
    //aRenamed.myText(nIM).prop({title:formatTitleStr(arrSourceIM)});
    ...
    aCreated.myText(nCreate).prop({title:formatTitleStr(arrCreate)}); spanCreatedAction.myText(nCreate);
    aDeleted.myText(nDelete).prop({title:formatTitleStr(arrDelete)}); spanDeletedAction.myText(nDelete);
    ...
    //aChanged.myText(nChanged).prop({title:formatTitleStr(arrSourceChanged)});  spanChangedAction.myText(nChanged)
...
gThis.divTabT2DICreator=function(el, charSide){
  ...
  el.setVal=function(syncDb){
    ...
    //aRenamed.myText(nIM).prop({title:formatTitleStr(arrSourceIM)});
    spanRenamedAction.myText(nIM)
    aCopiedNRenamedToOrigin.myText(nNM).prop({title:formatTitleStr(arrSourceNM)});  spanCopiedNRenamedToOriginAction.myText(nNM)
    aChanged.myText(nChanged).prop({title:formatTitleStr(arrSourceChanged)});  spanChangedAction.myText(nChanged)
    aReusedName.myText(nReusedName).prop({title:formatTitleStr(arrSourceReusedName)});  spanReusedNameAction.myText(nReusedName)
    aReusedId.myText(nReusedId).prop({title:formatTitleStr(arrSourceReusedId)});  spanReusedIdAction.myText(nReusedId)
    aCreated.myText(nCreate).prop({title:formatTitleStr(arrCreate)}); spanCreatedAction.myText(nCreate);
    aDeleted.myText(nDelete).prop({title:formatTitleStr(arrDelete)}); spanDeletedAction.myText(nDelete);

    var arrTmp=[arrSourceM1T1.slice(0,nShortListMax), arrTargetM1T1.slice(0,nShortListMax)];  arrTmp=Mat.transpose(arrTmp)
    aM1T1.myText(nM1T1).prop({title:formatTitleStr(arrTmp, fun1T1)});
    //aM1T1.myText(nM1T1).prop({title:formatTitleStr(arrSourceM1T1)});
    span1T1Action.myText(nM1T1);
    var strT=nSourceMMult+' \\ '+nTargetMMult;
...
gThis.divFolderInfoCreator=function(el, ...Fun){
  ...
  el.setVal=function(syncT2T){
    ...
    aCreatedF.myText(`${nCreateF}`).prop({title:formatTitleStr(arrCreateF)})
    aDeletedF.myText(`${nDeleteF}`).prop({title:formatTitleStr(arrDeleteF)})
  }
