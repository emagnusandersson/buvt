...
//             divAreaCatPrim2 (divAreaCatPrimCreator)
...
//         divAreaCatPrim1 (divAreaCatPrimCreator)


/***********************************************************
 * divIdMatchCreator, divAreaCatPrimCreator
 ***********************************************************/
...
If you still have hard-links among your files, I suggest you use the filter function to skip them.`
  ...
  el.myAppend(bHLCheck, ', folders: ', aMultF, ', files: ', aMult, ' ', spanHLCheckResult); // failures:   , ', ', spanLab, ': ', spanMultSum, elWrap
...
gThis.divOwnCounterCreator=function(el, ...Fun){
  el.clearVal=function(){
    spanOwn.myText('-'); spanUpstream.myText('-');
    spanOwnDiff.myText('-'); spanUpstreamDiff.myText('-');
    spanUpstreamLab.prop({title:undefined})
  }
  el.setVal=function(objOwnCounter){
    var {nTrOwn, nTrUpstream, nDbOwn, nDbUpstream, FleSub, boDataInTop}=objOwnCounter;
    ...
    spanUpstream.myText(nTrUpstream);
    var nDiff=nTrUpstream-nDbUpstream, strDiff=nDiff.toString(); if(nDiff>0) strDiff='+'+strDiff; else if(nDiff==0) strDiff='±'+strDiff;
    spanUpstreamDiff.myText(strDiff);

    // var nMax=20, text=FleSub.join(', ');
    // if(text.length>nMax) text=text.slice(0,nMax)+='…';
    // if(text.length) text=', '+text
    var title=FleSub.length?FleSub.join('\n'):undefined;   spanUpstreamLab.prop({title})
  }

  var spanOwn=createElement('span').myText('-'), spanUpstream=createElement('span').myText('-');
  var spanUpstreamLab=createElement('span').myText('Upstream entries');
  var spanOwnDiff=createElement('span').myText('-'), spanUpstreamDiff=createElement('span').myText('-');
  //var span=createElement('span')

  el.myAppend('Own entries: ', spanOwn, '(', spanOwnDiff, '), ', spanUpstreamLab, ': ', spanUpstream, '(', spanUpstreamDiff, ') ');
  return el
}


gThis.divDbRelationTestCreator=function(el, fun){ // find_1MToM_SMHash
  el.clearVal=function(){
  ...
  var title=`Testing that the own db-entries' SM-hash-relations are 1M-To-1 (1-To-1 or Many-To-1) and not 1M-To-M (1-To-Many or Many-To-Many).
In other words, checking that if multiple files have the same SM, then they must also have the same hash.
Files that fails this test (files with the same SM but at least one of them has a different hash) are listed to the right.`
  var bDbOwnCheck=createElement('b').myText(`Testing if own db-entries' SM can be used as keys to own db-entries' hashcodes.`).prop({title}); //1M-To-M test
  // Testing own db-entries' SM-keys
//So in other words only 1-to-1 or Many-to-1 (1M-To-1) SM-hash relations are allowed.
// 
// Db 1-To-Many or Many-To-Many (1M-To-M) SM-hash relation test
// Checking (own) files of db so that there is no 1M-To-M SM-hash relations.
...
gThis.menuCatPrimCreator=function(el, ...Fun){
  el.clearVal=function(){
    ...
    spanIntersection.myText(`-`); spanUnion.myText(`-`);
  }
  el.setVal=function(MatLoc){
    ...
    var strT=`${nPatY01} (∅ \\ =)`; span01.myText(strT)
    ...
    var strT=`${nPatY10} (= \\ ∅)`; span10.myText(strT)
    var strT=`${nPatY11} (= \\ =)`; span11.myText(strT); but11.myText(strT)
    ...
    var nA_4_Sum=nA11+nA12+nA21+nA22, nB_4_Sum=nB11+nB12+nB21+nB22
    spanSumS.myText(`${nAPat} (${nA} \\ ${nB_4_Sum})`); spanSumT.myText(`${nBPat} (${nA_4_Sum} \\ ${nB})`);

    var nPatBoth=nPatY11+nPatY12+nPatY21+nPatY22;  spanIntersection.myText(nPatBoth);
    spanUnion.myText(nPat);
  }

  var htmlHead=`<tr><th class=backcrossed><span title=Tree>Tr</span> <span>Db</span></th> <th>0</th> <th>1</th> <th>Many</th> <td>∑</td></tr>`; //S⇣ \\ T⇾
  var htmlBody=
  `<tr><th>0</th><td class=crossed><img/></td><td><img/><span>-</span></td> <td><img/><span>-</span><a>-</a></td> <th><span>-</span></th> <td/></tr>
  <tr><th>1</th><td><img/><span>-</span></td><td><img/><span>-</span><a>-</a></td>  <td><img/><span>-</span><a>-</a></td> <th><span>-</span></th> <th rowspan=2>Tr: <span>-</span></th></tr>
  <tr><th>Many</th><td><img/><span>-</span><a>-</a></td>  <td><img/><span>-</span><a>-</a></td>  <td><img/><span>-</span><a>-</a></td>  <th><span>-</span></th></tr>
  <tr><td>∑</td><th><span>-</span></th><th><span>-</span></th><th><span>-</span></th> <th rowspan=2 colspan=2>Tr ⋂ Db: <span>-</span><br/>Tr ⋃ Db: <span>-</span></th></tr>
  <tr><td/><td/><th colspan=2>Db: <span>-</span></th></tr>` //∪∩⋂⋃ (∞)
  // title="Number of patterns that occur on both sides."    title="Total number of patterns"
  ...
  var [tr0, tr1, tr2, trSum, trSumT]=tBody.children
  ...
  var arr1=tr1.querySelectorAll('span'), [span10,span11,span12, span1XSum, spanSumS]=arr1; //, spanSumPatS
  ...
  var [spanX0Sum, spanX1Sum, spanX2Sum, spanIntersection, spanUnion]=trSum.querySelectorAll('span')
  var [spanSumT]=trSumT.querySelectorAll('span'); //spanSumPatT, 
  ...
  [...Img0, ...Img1, ...Img2].forEach(ele=>ele.css({zoom:0.4, display:'block'})); Img0[0].hide();
  ...
  var strHelp=`Number of SM patterns that exist in Tree and Db. <img/>`;
  //⋂: Intersection of patterns in Tree resp Db <br/>
  var divHelp=divHelpProt.cloneNode(1).myHtml(strHelp).css({'max-width':'28em'});
  var imgHelp=hovHelp.cloneNode(1).css({margin:'0 0 0 .4em'}); popupHover(imgHelp, divHelp);
  var imgI=divHelp.querySelector('img');
  spanIntersection.myAfter(imgHelp);
  var strHelp=`Number of SM patterns that exist in Tree or Db (or both). <img/>`;
  var divHelp=divHelpProt.cloneNode(1).myHtml(strHelp).css({'max-width':'32em'});
  var imgHelp=hovHelp.cloneNode(1).css({margin:'0 0 0 .4em'}); popupHover(imgHelp, divHelp);
  var imgU=divHelp.querySelector('img');
  spanUnion.myAfter(imgHelp)
  var fleImgT="icons/",  LeafImg=['intersection.png', 'union.png'];
  [imgI, imgU].forEach(   (ele, i)=>ele.attr({src:`${fleImgT}${LeafImg[i]}`}).css({zoom:0.7, 'vertical-align':'middle'})   );

//   var strHelp=`Explanation of numbers in the table:<br/>\n
// nPattern (nEntriesTree \\ nEntriesDb)`

  var strHelp=`Explanation of numbers in the table:<br/>
<b>nPattern</b> (<b>nEntriesTree</b> \\ <b>nEntriesDb</b>)
<dl>
<dt>nPattern</dt><dd>number of SM patterns.</dd>
<dt>nEntriesTree</dt><dd>number of entries (files, soft links) in Tree.</dd>
<dt>nEntriesDb</dt><dd>number of entries (files, soft links) in Db.</dd>
<dt>∅</dt><dd>the number is always zero</dd>
<dt>=</dt><dd>the number is always the same as nPattern</dd>
</dl>`
  var divHelp=divHelpProt.cloneNode(1).myHtml(strHelp).css({'max-width':'32em'})
  var imgHelp=hovHelp.cloneNode(1).css({margin:'0em 0em'}); popupHover(imgHelp, divHelp);
  el.myAppend(imgHelp, table);  
  return el
}

gThis.divAreaCatPrimCreator=function(el, ...Fun){
  el.clearVal=function(){
  ...
  var strHelp=`Explanation of numbers:<br/>\n
nPatternTree (nEntryTree) \\ nPatternDb (nEntryDb)
<dl>
<dt>nPatternTree</dt><dd>number of SM patterns in Tree.</dd>
<dt>nEntryTree</dt><dd>number of entries (files, soft links) in Tree.</dd>
<dt>nPatternDb</dt><dd>number of SM patterns in Db.</dd>
<dt>nEntryDb</dt><dd>number of entries (files, soft links) in Db.</dd>
</dl>`
  var divHelp=divHelpProt.cloneNode(1).myHtml(strHelp).css({'max-width':'24em'})
  var imgHelp=hovHelp.cloneNode(1).css({margin:'0em .4em'}); popupHover(imgHelp, divHelp);

  el.myAppend(elWrap, ' ', aOpenList, imgHelp); //, ', ', spanLab, ': ', spanMultSum
  //el.css({display:'inline'})
  return el
}
...
On the target side, the files are NOT filtered at all.`})
