...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  butDoAction.on('click', async function(){
    ...
    var {fiSourceDir, fiTargetDbDir, fiTargetDataDir, charTResS, charTResT, charFilterMethod, strHostTarget:strHost}=result
    //var fiTargetDbDir=fiTargetDbDir||fiTargetDataDir
    var boTarget=charSide=='T'
    var fiDir=boTarget?fiTargetDataDir:fiSourceDir
...
gThis.divT2DCreator=function(el, charSide){
  ...
  var funT2D=async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDbDir, fiTargetDataDir, charTResS, charTResT, charFilterMethod, strHostTarget, boIncIdS, boIncIdT}=result

    if(boTarget){
      

      var fiDbDir=fiTargetDbDir, fiDir=fiTargetDataDir, charTRes=charTResT;
      //var fiDir=fiTargetDbDir;  // Old way
      var boIncId=el.boIncId=boIncIdT;
      var strHost=strHostTarget;
    }else{
      var fiDbDir=fiSourceDir, fiDir=fiSourceDir, charTRes=charTResS;
...
gThis.divT2TUsingHashCreator=function(el){
  ...
  var funT2T=async function(){
    ...
    //if(!fiTargetDbDir) fiTargetDbDir=fiTargetDataDir
    ...
    //copySome(arg, result, ["fiSourceDir", "fiTargetDbDir", "fiTargetDataDir", "strHostTarget", "boAllowLinks", "boAllowCaseCollision", "strTargetCharSet"])
