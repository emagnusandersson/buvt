...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  butDoAction.on('click', async function(){
    ...
    var {fiSourceDir, fiTargetDbDir, flTargetDataDir, charTResS, charTResT, charFilterMethod, strHostTarget:strHost}=result
    var boTarget=charSide=='T'
    var fiDbDir=boTarget?fiTargetDbDir:fiSourceDir
    var charTRes=boTarget?charTResT:charTResS;
    if(!boTarget) {strHost="localhost"; }
    ...
    var [err, fsDbDir]=await myRealPath(fiDbDir, strHost); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var fsDir=fsDbDir; if(boTarget && flTargetDataDir) {debugger; fsDir=fsDbDir+charF+flTargetDataDir; }
    
    var arg={leafFilter, fsDir, fsDbDir, charTRes, charFilterMethod, strHost, charSide}
...
gThis.divT2DCreator=function(el, charSide){
  ...
  var funT2D=async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDbDir, flTargetDataDir, charTResS, charTResT, charFilterMethod, strHostTarget}=result; //, boIncIdS, boIncIdT

    if(boTarget){
      var fiDbDir=fiTargetDbDir, charTRes=charTResT;
      var strHost=strHostTarget;
    }else{
      var fiDbDir=fiSourceDir, charTRes=charTResS;
    ...
    var fsDir=fsDbDir; if(boTarget && flTargetDataDir) fsDir=fsDbDir+charF+flTargetDataDir
    var arg={leafFilter, fsDir, fsDbDir, charTRes, charFilterMethod, strHost, charSide}
...
//   var [aM1T1, butMore]=But
  ...
  var divTabW=createElement('div').myAppend(divTab)  //, divTabI
...
gThis.divT2TUsingHashCreator=function(el){
  ...
  var funT2T=async function(){
    ...
    var arg={fsSourceDir, fsTargetDbDir, fsTargetDataDir, leafFilter, leafFilterFirst, charTRes, charFilterMethod};
    copySome(arg, result, ["strHostTarget", "boAllowLinks", "boAllowCaseCollision", "strTargetCharSet"])
