...
gThis.divT2DBothCreator=function(el){
  ...
  for(var i=0;i<2;i++){
    ...
    var butUniquify=createElement('button').myAppend('Uniquify SM relative to hashcodes').prop({title:`Note! this does not mean that individual files can't have the same SM)\nCopies (same hashcode) may still have the same SM)`}).css({margin:'auto'}).on('click', funUniquifySM).disable()
