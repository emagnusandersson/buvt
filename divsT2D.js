...
gThis.divT2DBothCreator=function(el){
  ...
  var funCompare=async function(){
    ...
    for(var iSide of ISide){
      ...
      var {objHL}=syncDb;   if(objHL) {el.setHLVal(objHL, iSide);}
      el.setVal(syncDb.objCategory, iSide);
      arrSyncDb[iSide]=syncDb;
      SyncT2D.fun1_5CreateNewDb(arrSyncDb[iSide]);
  ...
  var funUniquifySM=async function(){
    ...
    for(var iSide of ISide){
      ...
      var {BundXTM}=result
      el.MiniViewSMMatch[boTarget].setUI_SMMultiples(result);
      ...
      el.MiniViewSMMatch[boTarget].setUI_NewMTime(result)
  ...
  var funWriteToDb=async function(){
    ...
    for(var iSide of ISide){
      ...
      var [err]=await SyncT2D.fun4WriteDb(ArrToChange[iSide], arrSyncDb[iSide], argGeneral, boTarget); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); EnumStat[iSide]=EnumStatT.none; funSetSingleButton(iSide); return;}  //butFinish.disable();
      EnumStat[iSide]=EnumStatT.none;
      funSetSingleButton(iSide); funSetBothButtons();
      arrSyncDb[iSide]=undefined; ArrToChange[iSide]=undefined;
    }
  ...
  for(var i=0;i<2;i++){
    ...
    var headSMHashFixing=createElement('div').myAppend('Fix SM-hash-relations').prop({title:`Get rid of any XTM (1TM and MTM) relations (see table below).`})
