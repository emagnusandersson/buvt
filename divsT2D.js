...
gThis.divT2DBothCreator=function(el){
  ...
  var funCompare=async function(){
    ...
    for(var iSide of ISide){
      ...
      arrSyncDb[iSide]=new SyncDbOne({divT2DBoth:el, charSide, argGeneral})
      var [err]=await arrSyncDb[iSide].fun1Prework(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;};
  ...
  for(var i=0;i<2;i++){
    ...
    var butCompareT2D=createElement('button').myAppend('Compare own SM').prop({title:"Corresponding to the green area in the little image."}).on('click', funCompare);
    var linkFilterFirstT2D=createElement('a').myText('filterFirst').prop({ href:""}).on('click', methGoToTitle);
    var divButton=createElement('div').myAppend(hT2D, ' ', butCompareT2D, ' ', linkFilterFirstT2D).css({background:'var(--bg-color)', flex:"0 1", top:0, border:"solid 1px", position:'sticky'}); //, opacity:0.8
    ...
    var butUniquify=createElement('button').myAppend('Uniquify').css({margin:'auto'}).on('click', funUniquifySM).disable()
    var divUniquify=createElement('div').myAppend(butUniquify);  divUniquify.setAttribute('data-side', charSide);
  ...
  var divUniquifySM=createElement('div').myAppend(butUniquifySM).css({background:'var(--bg-color)', flex:"0 1", top:0, border:"solid 1px", 'text-align':'center', 'grid-row':'span 2'}); //, 'grid-column':'span 3', 'grid-area':'1/1/span 1/span 2'   title:'Uniquify SM', 
  ...
  el.myAppend(...DivButton, divCompareBoth, divGo, ...DivIdMatch, ...DivMat1, ...DivTabW, ...DivCalcHash, divCalcHash, ...DivUniquify, divUniquifySM, ...MiniViewSMMatch, ...DivFinish); // , divFinish, ...DivDbWrite
