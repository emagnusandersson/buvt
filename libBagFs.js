...
var formatMatchingDataWMult=function(ArrS, ArrT){
  ...
  for(var i in ArrS){
    ...
    var rowT
    if(nS) {var s0=arrS[0], sE=arrS[nS-1], boSEq=s0.mtime_ns64==sE.mtime_ns64, rowT=s0;} else boSEq=true
    if(nT) {var t0=arrT[0], tE=arrT[nT-1], boTEq=t0.mtime_ns64==tE.mtime_ns64, rowT=t0;} else boTEq=true
    if(nS && nT) var bo0Eq=s0.mtime_ns64==t0.mtime_ns64; else bo0Eq=true
    var boAllEq=boSEq && boTEq && bo0Eq

    var strMTimeExact=(boAllEq && rowT.mtime_ns64!=rowT.mtime_ns64R)?` ${rowT.strMTime.padStart(19)}`:'seeRounded'
    var strLab=`MatchingData ${rowT.size.myPadStart(10)} ${rowT.strMTimeR.padStart(19)} ${strMTimeExact}`
    arrOut.push(strLab);
  ...
  var formatMatchingDataWMultSingleDataSetST=function(Arr, strSide){
    ...
    for(var i in Arr){
      ...
      var strMTimeExact=(boAllEq && r0.mtime_ns64!=r0.mtime_ns64R)?` ${r0.strMTime.padStart(19)}`:'seeRounded'
      var strLab=`MatchingData ${r0.size.myPadStart(10)} ${r0.strMTimeR.padStart(19)} ${strMTimeExact}`
      arrOut.push(strLab);
...
var parseMultSTFile=async function(fiInpFile){
  ...
  for(var i in arrInp){
    ...
    if(strFirst=='MatchingData'){
      ...
      var size=parseInt(arrPartMatching[0]), strMTimeR=arrPartMatching[1], mtime_ns64R=BigInt(strMTimeR)
      ...
      if(strMTimeTmp=='seeAbove'){ var strMTime=strMTimeExact=='seeRounded'?strMTimeR:strMTimeExact; }  else  { var strMTime=strMTimeTmp; }
      var mtime_ns64=BigInt(strMTime)
      var row={strType, size, id, mtime_ns64, mtime_ns64R, strName, strMTime, strMTimeR}
...
var formatRename1T1=function(arrA11, arrB11){
  ...
  var funMatch=(s,t)=>{
    return `MatchingData ${s.id} ${s.size.myPadStart(10)} ${s.strMTimeR.padStart(19)}`;
  }
  var funUniqueS=s=>{
  ...
  var strHeadMT=`string int string`, strHeadM=`id size strMTimeR`;
