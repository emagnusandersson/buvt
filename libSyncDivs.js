...
gThis.divHashMatchCreator=function(el, fun){
  ...
  el.setVal=function(syncDb){
    ...
    aMult.myText(`${nHashMult}(${nPatHashMult})`)
  }
  ...
  var aMult=createElement('a').prop({href:''}).myText('-').on('click',fun);

  el.myAppend(spanLab, ': Total: ', spanTotal, ', Mult: ', aMult); // elWrap
  //el.css({display:'inline'})
  return el
}
...
gThis.divTabT2DCreator=function(el, charSide){
  var self=el
  el.clearVal=function(){
  ...
  el.setVal=function(syncDb){
    ...
    var {Mat1, Mat2, boChanged, arrSource, arrTarget, arrUntouched, arrSourceUntouched, arrTargetUntouched, arrCreate, arrDelete, arrSourceM1T1, arrTargetM1T1}=syncDb; //, arrSourceMMult, arrTargetMMult
    ...
    //aRenamed.myText(nIM).prop({title:formatTitle(arrTmp, undefined, fun1T1)});
    //aRenamed.myText(nIM).prop({title:formatTitle(arrSourceIM)});
    ...
    aM1T1.myText(nM1T1).prop({title:formatTitle(arrTmp, undefined, fun1T1)});
    ...
    aCreated.myText(nCreate).prop({title:formatTitle(arrCreate)}); spanCreatedAction.myText(nCreate);
    aDeleted.myText(nDelete).prop({title:formatTitle(arrDelete)}); spanDeletedAction.myText(nDelete);
    ...
    //aChanged.myText(nChanged).prop({title:formatTitle(arrSourceChanged)});  spanChangedAction.myText(nChanged)
...
<tr><th>Na&shy;me</th> <th title="Size and Modification time">SM</th> <th>Un­touch­ed / Short­cut</th> <th>Del­ete</th> <th title="(Re-) Calculate the hashcode for the file ">Calc</th></tr>`
...
<tr><td colspan=8>  <div class=dupEntry>SM-Combos: </div>  </td></tr>
<tr><th>-</th><th>1T1</th> <td colspan=2><a href="">-</a></td><td>Re­named</td>  <td><span>-</span></td> <td colspan=2 class=crossed></td></tr>
...
// The pattern exists on both S and T and more than once on at least one of them. <br/>(read fr files)
  ...
  var [trUntouched, trMat2, tr1T1, trMult, trCreatedNDeleted, trSum, trChanged, trSyncAccordingToFiles]=arrTR; //trMat1, 
...
gThis.divTabT2DICreator=function(el, charSide){
  var self=el
  el.clearVal=function(){
    butDoActionRam.prop({disabled:true});   butDoAction.prop({disabled:true})
    divMat2.clearVal()
    ...
    aCreated.myText('-').prop({title}); aDeleted.myText('-').prop({title});
    aM1T1.myText('-').prop({title}); aMult.myText('-').prop({title});
    spanDeletedAction.myText('-');spanCreatedAction.myText('-')
    spanSumS.myText('-');  spanSumT.myText('-')
    spanSumSC.myText('-');  spanSumDelete.myText('-');  spanSumCreate.myText('-');
  }
  el.setVal=function(syncDb){
    ...
    var {Mat1, Mat2, boChanged, arrSource, arrTarget, arrSourceUntouched, arrTargetUntouched, arrSourceChanged, arrTargetChanged, arrSourceIM, arrTargetIM, arrSourceNM, arrTargetNM, arrSourceReusedName, arrTargetReusedName, arrSourceReusedId, arrTargetReusedId, arrCreate, arrDelete, arrSourceM1T1, arrTargetM1T1}=syncDb; //, arrSourceMMult, arrTargetMMult
    ...
    //aRenamed.myText(nIM).prop({title:formatTitle(arrSourceIM)});
    spanRenamedAction.myText(nIM)
    aCopiedNRenamedToOrigin.myText(nNM).prop({title:formatTitle(arrSourceNM)});  spanCopiedNRenamedToOriginAction.myText(nNM)
    aChanged.myText(nChanged).prop({title:formatTitle(arrSourceChanged)});  spanChangedAction.myText(nChanged)
    aReusedName.myText(nReusedName).prop({title:formatTitle(arrSourceReusedName)});  spanReusedNameAction.myText(nReusedName)
    aReusedId.myText(nReusedId).prop({title:formatTitle(arrSourceReusedId)});  spanReusedIdAction.myText(nReusedId)
    aCreated.myText(nCreate).prop({title:formatTitle(arrCreate)}); spanCreatedAction.myText(nCreate);
    aDeleted.myText(nDelete).prop({title:formatTitle(arrDelete)}); spanDeletedAction.myText(nDelete);

    var arrTmp=[arrSourceM1T1.slice(0,nShortListMax), arrTargetM1T1.slice(0,nShortListMax)];  arrTmp=Mat.transpose(arrTmp)
    aM1T1.myText(nM1T1).prop({title:formatTitle(arrTmp, undefined, fun1T1)});
    //aM1T1.myText(nM1T1).prop({title:formatTitle(arrSourceM1T1)});
    span1T1Action.myText(nM1T1);
    var strT=nSourceMMult+' \\ '+nTargetMMult;
...
<tr><th>Id</th> <th>Na&shy;me</th> <th title="Size and Modification time">SM</th> <th>Un­touch­ed / Short­cut</th> <th>Del­ete</th> <th title="(Re-) Calculate the hashcode for the file ">Calc</th></tr>`
...
gThis.divT2DCreator=function(el, charSide){
  ...
  var funT2D=async function(){
    ...
    var [err, result]=await argumentTab.getSelectedFrFile(); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var {fiSourceDir, fiTargetDir, fiTargetDbDir, charTResS, charTResT, charFilterMethod, strHostTarget:strHost, boRemote, boIncIdS, boIncIdT}=result
    //var fiTargetDbDir=fiTargetDbDir||fiTargetDir
    var boTarget=charSide=='T'
    var fiDir=boTarget?fiTargetDbDir:fiSourceDir
    var fiDbDir=boTarget?fiTargetDbDir:fiSourceDir
    var charTRes=boTarget?charTResT:charTResS;
    if(!boTarget) {strHost="localhost"; boRemote=false;}
    var boIncId=el.boIncId=boTarget?boIncIdT:boIncIdS;
    //divTabI.toggle(boIncId); divTab.toggle(!boIncId)
    var leafFilter=LeafFilter[charFilterMethod];
    var [err, fsDir]=await myRealPath(fiDir); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var [err, fsDbDir]=await myRealPath(fiDbDir); if(err) {debugger; myConsole.error(err); resetMess(); blanket.hide(); return;}
    var arg={leafFilter, fsDir, fsDbDir, charTRes, charFilterMethod, strHost, boRemote, charSide}
  ...
  var divTab=el.divTab=divTabT2DCreator(createElement('div'), charSide)
  var divTabI=el.divTabI=divTabT2DICreator(createElement('div'), charSide)
...
gThis.divFolderInfoCreator=function(el, ...Fun){
  ...
  el.setVal=function(syncT2T){
    ...
    aCreatedF.myText(`${nCreateF}`).prop({title:formatTitle(arrCreateF)})
    aDeletedF.myText(`${nDeleteF}`).prop({title:formatTitle(arrDeleteF)})
  }
...
gThis.divT2TUsingHashCreator=function(el){
  ...
  el.myAppend(divButton, divFolderInfo, divTab); // divConsoleT2D, divConsoleT2T, divConsole, divMat1
